...
//! `agentpalace_storage`/`agentpalace_federation` yet (no auth-scheme description, no signing key,
...
use agentpalace_federation::InfoResponse;
...
pub struct AgentCapabilities {
    /// Whether the agent supports streaming responses. AgentPalace coordination is poll-based
    ...
    /// Protocol extensions supported by the agent. Always empty — AgentPalace declares none.
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub extensions: Vec<AgentExtension>,
}

/// A declared protocol extension, per A2A's `AgentExtension`. AgentPalace declares none today;
...
pub fn build_agent_card(inputs: &AgentCardInputs<'_>) -> AgentCard {
    let skills = inputs
        ...
        .map(|wing| AgentSkill {
            ...
            name: wing.clone(),
            description: format!(
                "Coordinate tasks in the `{wing}` wing via AgentPalace's coordination surface."
            ),
...
mod tests {
    use agentpalace_federation::{FEDERATION_API_VERSION, MaintenanceStatus};

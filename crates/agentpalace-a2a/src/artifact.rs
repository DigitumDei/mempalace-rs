...
//! # Isolation
//!
//! `agentpalace_storage::coordination::Artifact` stores its body as an opaque `content: String`
...
use agentpalace_storage::{Artifact, NewArtifact};
...
///
/// `task_id` is the owning AgentPalace task; `created_by`/`idempotency_key` are resolved by the
/// caller the same way every other coordination write resolves them (see
/// `agentpalace_storage::coordination::NewArtifact`).
///

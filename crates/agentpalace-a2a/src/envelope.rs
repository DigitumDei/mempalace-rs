...
//!
//! No A2A field may become a column in AgentPalace's core schema (see the crate-level docs).
...
//! `agentpalace_storage::coordination`, keeps the core schema untouched, and keeps the exchange
//! auditable: the original A2A state is always recoverable from the envelope even when a
//! coercion changed what AgentPalace recorded as the task's own state.
...
/// same way `agentpalace_storage::coordination::NewArtifact::created_by` is resolved everywhere
/// else).
///
/// # Idempotency key derivation
///
/// `idempotency_key` is `"a2a_envelope:{task_id}:{hash}"`, where `hash` is the full BLAKE3 hex
/// digest of `envelope_json`'s raw bytes, computed the same way `agentpalace_storage::coordination`

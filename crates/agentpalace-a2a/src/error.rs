...
/// Errors raised while translating between A2A wire shapes and AgentPalace storage types.
#[derive(Debug, thiserror::Error)]
pub enum A2aError {
    /// The A2A task carried `UNSPECIFIED`, which the design explicitly forbids coercing into
    /// any AgentPalace state (see `docs/Coordination-Phase-3-Design.md`, Stage 5). This is the
    /// caller's malformed input, not a mapping the adapter can complete.
    #[error(
        "A2A task state `UNSPECIFIED` is malformed and cannot be mapped to a AgentPalace task state"
    )]

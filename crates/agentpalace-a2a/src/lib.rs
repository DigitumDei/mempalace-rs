...
//! Foundation, 2026) and AgentPalace's local coordination storage.
//!
//! This crate is a **pure translation layer**: it adds no HTTP routes, no MCP tools, and no
//! config fields. It depends only on [`agentpalace_storage`] and [`agentpalace_federation`] plus a
...
//! # Isolation rule
//!
//! No A2A field may become a column in AgentPalace's core schema. Fields with no internal home
...
//! `agentpalace_storage`, so their A2A mappings ([`message`], [`artifact`]) apply the same "store
//! verbatim" principle directly rather than needing a separate envelope.
//!
//! # State mapping
//!
//! A2A defines nine task states against AgentPalace's seven, so the mapping is not bijective in

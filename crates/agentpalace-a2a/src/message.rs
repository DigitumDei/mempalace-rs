...
//! # Isolation
//!
//! `agentpalace_storage::coordination::Message` already stores its body as an opaque
...
use agentpalace_storage::{Message, NewMessage};
...
///
/// `task_id` is the owning AgentPalace task; `sender`/`recipient`/`idempotency_key` are resolved
/// by the caller the same way every other coordination write resolves them (see
/// `agentpalace_storage::coordination::NewMessage`). `envelope_version` defaults to `1`, matching

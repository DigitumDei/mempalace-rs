...
//!
//! A2A v1.0 defines nine task states; AgentPalace has seven. The mapping is documented in
//! `docs/Coordination-Phase-3-Design.md` (Stage 5):
//!
//! | A2A | AgentPalace | Note |
//! |---|---|---|
...
/// Map an inbound A2A task state to its AgentPalace counterpart.
///
/// Every row of the table in the module docs is covered. `UNSPECIFIED` is rejected outright —
/// see [`A2aError::UnspecifiedTaskState`] — rather than coerced to any AgentPalace state, per the
...
/// Map an outbound AgentPalace task state to its A2A counterpart.
///
/// Inbound is not total in the AgentPalace direction ([`TaskState::Expired`] has no A2A

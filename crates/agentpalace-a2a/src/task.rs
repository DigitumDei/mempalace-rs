...
//! concept at all — wings are a AgentPalace-specific multi-tenancy boundary), or who created it
...
//! AgentPalace, and the A2A `Task` message carries no analogue for any of them (no parent-task
...
//! AgentPalace's own `coordination_artifacts`/`coordination_messages` rows via
...
/// The result of translating an inbound A2A `Task` into a AgentPalace [`NewTask`].
#[derive(Debug, Clone)]
pub struct NewTaskConversion {
    ...
    /// whether reaching it required coercing an A2A state AgentPalace has no direct counterpart
...
/// Translate an outbound AgentPalace [`Task`] into an [`A2aTask`].
///
...
/// [`Task`] itself carries neither. `status_message` becomes `TaskStatus.message`; AgentPalace's
...
///
/// `context_id` and `metadata` are always `None`: AgentPalace's `Task` has no A2A "context" grouping

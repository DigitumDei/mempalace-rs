...
//
// Usage:
//   cargo build --example lme_bench --release -p agentpalace-cli
...
use agentpalace_core::{EmbeddingProfile, SearchQuery};
use agentpalace_embeddings::{FastembedProvider, FastembedProviderConfig};
use agentpalace_ingest::{ProjectIngestRequest, ingest_project};
use agentpalace_search::SearchRuntime;
use agentpalace_storage::StorageEngine;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let allow_downloads = agentpalace_core::env_var("AGENTPALACE_EMBED_ALLOW_DOWNLOADS")
    ...
    for (idx, entry) in entries.iter().enumerate() {
        ...
        fs::write(
            sessions_dir.join("agentpalace.yaml"),
        ...
        let query =
            SearchQuery { text: question, wing: None, room: None, limit: 10, profile, view: None };
        let search_results =

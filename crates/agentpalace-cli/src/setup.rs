//! `agentpalace setup` — detect installed AI coding tools and register the
//! agentpalace MCP server with each, idempotently.
//!
//! The MCP server is the local `agentpalace` binary (by default
...
pub struct SetupOptions {
    /// Override for the agentpalace binary path. When `None`, defaults to
...
impl SetupReport {
    ...
    pub fn render(&self) -> String {
        ...
        if !self.mcp_binary_present {
            ...
            lines.push("    install agentpalace there for them to launch it.".to_owned());
        }
...
fn handle_tool(key: &'static str, mcp: &str, dry_run: bool) -> ToolOutcome {
    if mcp.is_empty() {
        return outcome(
            key,
            Status::Failed,
            "could not resolve a agentpalace path (no home directory; pass --mcp-path)",
        );
    }
    match key {
        "jules" => {
...
fn configure_claude(bin: &Path, mcp: &str, dry_run: bool) -> ToolOutcome {
    ...
    if dry_run {
        return outcome(
            "claude",
            Status::WouldConfigure,
            format!("would run: claude mcp add --scope user agentpalace -- {mcp} serve --stdio"),
        );
    }
    // `claude mcp get` exits non-zero when the server is absent.
    if let Ok(out) = run_tool(bin, &["mcp", "get", "agentpalace"]) {
        if out.status.success() {
            return outcome("claude", Status::AlreadyConfigured, "already registered (user scope)");
        }
    }
    match run_add_with_config(
        bin,
        &["mcp", "add", "--scope", "user", "agentpalace", "--", mcp, "serve", "--stdio"],
    ) {
...
fn configure_codex(bin: &Path, mcp: &str, dry_run: bool) -> ToolOutcome {
    ...
    if dry_run {
        return outcome(
            "codex",
            Status::WouldConfigure,
            format!("would run: codex mcp add agentpalace -- {mcp} serve --stdio"),
        );
    }
    if let Ok(out) = run_tool(bin, &["mcp", "get", "agentpalace"]) {
        if out.status.success() {
    ...
    match run_add_with_config(bin, &["mcp", "add", "agentpalace", "--", mcp, "serve", "--stdio"]) {
        Ok(out) if out.status.success() => {
...
fn configure_gemini(bin: &Path, mcp: &str, dry_run: bool) -> ToolOutcome {
    ...
    if dry_run {
        return outcome(
            "gemini",
            Status::WouldConfigure,
            format!("would run: gemini mcp add -s user agentpalace \"{mcp}\""),
        );
    }
    ...
    // false-positive on `agentpalace-old` or on the `~/.agentpalace/...` binary path.
    if let Ok(out) = run_tool(bin, &["mcp", "list"]) {
        if out.status.success()
            && list_contains_server(&String::from_utf8_lossy(&out.stdout), "agentpalace")
        {
            return outcome("gemini", Status::AlreadyConfigured, "already registered (user scope)");
        }
    }
    match run_add_with_config(bin, &["mcp", "add", "-s", "user", "agentpalace", "--", mcp, "serve", "--stdio"]) {
        Ok(out) if out.status.success() => {
...
/// it won't be fooled by `agentpalace-old`, `my-agentpalace`, or a command path that
...
fn run_add_with_config(bin: &Path, args: &[&str]) -> io::Result<std::process::Output> {
    let config_dir = agentpalace_core::env_var("AGENTPALACE_CONFIG_DIR").ok();
    let args = add_args_with_config(args, config_dir.as_deref());
    run_tool(bin, &args.iter().map(String::as_str).collect::<Vec<_>>())
}

fn add_args_with_config(args: &[&str], config_dir: Option<&str>) -> Vec<String> {
    let mut result: Vec<String> = args.iter().map(|s| (*s).to_owned()).collect();
    if let Some(dir) = config_dir.filter(|dir| !dir.trim().is_empty()) {
        if let Some(delimiter) = result.iter().position(|arg| arg == "--") {
            result.splice(delimiter..delimiter, ["--env".to_owned(), format!("AGENTPALACE_CONFIG_DIR={dir}")]);
        }
    }
    result
}
...
fn apply_json(
    ...
    let mut entry = entry;
    if let Ok(dir) = agentpalace_core::env_var("AGENTPALACE_CONFIG_DIR") {
        if !dir.trim().is_empty() {
            let env_key = if key == "opencode" { "environment" } else { "env" };
            entry[env_key] = json!({"AGENTPALACE_CONFIG_DIR": dir});
        }
    }
    let existing = match read_json(path) {
        ...
        Err(ReadError::Parse(e)) => {
            return outcome(
                ...
                Status::Failed,
                format!(
                    "{} is not valid JSON ({e}); add agentpalace manually to avoid clobbering it",
                    path.display()
                ),
            );
        }
    ...
    let (merged, changed) = upsert_mcp_entry(existing, top_key, "agentpalace", &entry);
    if !changed {
        return outcome(
            key,
            Status::AlreadyConfigured,
...
fn write_json(path: &Path, value: &Value) -> io::Result<()> {
    ...
    let tmp = path.with_extension("agentpalace-tmp");
    fs::write(&tmp, text)?;
    fs::rename(&tmp, path)
}
        ...
        if servers.get(name) != Some(entry) {
            ...
            if let (Some(existing), Some(updates)) = (merged.as_object_mut(), entry.as_object()) {
                for (key, value) in updates {
                    if (key == "env" || key == "environment" || key == "args") && existing.contains_key(key) {
                        continue;
                    }
                    if key == "command" {
                        if let (Some(old), Some(new)) = (existing.get(key).and_then(Value::as_array), value.as_array()) {
                            if !old.is_empty() && !new.is_empty() {
                                let mut command = old.clone();
                                command[0] = new[0].clone();
                                existing.insert(key.clone(), Value::Array(command));
                                continue;
                            }
                        }
                    }
...
fn legacy_launcher(command: &str) -> bool {
    matches!(command.rsplit(['/', '\\']).next(), Some("agentpalace-mcp" | "agentpalace-mcp.exe"))
}
...
fn migrate_json_launcher(
    ...
    let entry = document.get_mut("mcpServers")?.get_mut("agentpalace")?;
...
fn migrate_codex_at_path(path: PathBuf, mcp: &str, dry_run: bool) -> Option<ToolOutcome> {
    ...
    let entry = document.get_mut("mcp_servers")?.get_mut("agentpalace")?;
    if !legacy_launcher(entry.get("command")?.as_str()?) {
        return None;
    }
    ...
    let temporary = path.with_extension("agentpalace-tmp");
    Some(
...
mod tests {
    #[test]
    fn custom_home_is_one_environment_argument_before_the_launcher() {
        let args = ["mcp", "add", "agentpalace", "--", "/bin/agentpalace", "serve", "--stdio"];
        let updated = super::add_args_with_config(&args, Some("/home/with spaces/palace"));
        assert_eq!(updated, ["mcp", "add", "agentpalace", "--env", "AGENTPALACE_CONFIG_DIR=/home/with spaces/palace", "--", "/bin/agentpalace", "serve", "--stdio"]);
        assert_eq!(super::add_args_with_config(&args,None),args);
    }

    #[test]
    fn setup_after_migration_preserves_custom_arguments_and_environment() {
        let existing = serde_json::json!({"mcpServers":{"agentpalace":{
            "command":"/old/agentpalace","args":["serve","--stdio","--palace","/custom/palace"],
            "env":{"TOKEN":"secret","AGENTPALACE_CONFIG_DIR":"/custom/config"}}}});
        let replacement = serde_json::json!({"command":"/new/agentpalace","args":["serve","--stdio"],"env":{}});
        let (updated,changed) = super::upsert_mcp_entry(Some(existing.clone()),"mcpServers","agentpalace",&replacement);
        assert!(changed);
        assert_eq!(updated["mcpServers"]["agentpalace"]["args"],existing["mcpServers"]["agentpalace"]["args"]);
        assert_eq!(updated["mcpServers"]["agentpalace"]["env"],existing["mcpServers"]["agentpalace"]["env"]);
        assert_eq!(updated["mcpServers"]["agentpalace"]["command"],"/new/agentpalace");
    }
    ...
    fn migrations_preserve_environment_and_respect_dry_run() {
        ...
        let source = "# keep this comment\n[mcp_servers.agentpalace]\ncommand = 'C:/bin/agentpalace-mcp.exe'\n[mcp_servers.agentpalace.env]\nAGENTPALACE_LINEAGE_ID = 'default'\n";
        std::fs::write(&path, source).unwrap();
        assert_eq!(
            super::migrate_codex_at_path(path.clone(), "C:/bin/agentpalace.exe", true)
                .unwrap()
                .status,
        ...
        assert_eq!(
            super::migrate_codex_at_path(path.clone(), "C:/bin/agentpalace.exe", false)
                .unwrap()
                .status,
        ...
        assert_eq!(
            doc["mcp_servers"]["agentpalace"]["env"]["AGENTPALACE_LINEAGE_ID"].as_str(),
            Some("default")
        );
        assert_eq!(doc["mcp_servers"]["agentpalace"]["args"][0].as_str(), Some("serve"));
        assert!(super::migrate_codex_at_path(path, "C:/bin/agentpalace.exe", false).is_none());

        let path = temp.path().join("claude.json");
        std::fs::write(&path, r#"{"mcpServers":{"agentpalace":{"command":"/bin/agentpalace-mcp","env":{"AGENTPALACE_LINEAGE_ID":"default"}}},"theme":"dark"}"#).unwrap();
        super::migrate_json_launcher("claude", Some(path.clone()), "/bin/agentpalace", false)
            .unwrap();
        let doc: serde_json::Value =
            serde_json::from_str(&std::fs::read_to_string(path).unwrap()).unwrap();
        assert_eq!(doc["theme"], "dark");
        assert_eq!(doc["mcpServers"]["agentpalace"]["env"]["AGENTPALACE_LINEAGE_ID"], "default");
        assert_eq!(doc["mcpServers"]["agentpalace"]["args"], serde_json::json!(["serve", "--stdio"]));
    }
    ...
    fn upsert_creates_top_key_and_entry() {
        let entry = json!({ "command": "/bin/agentpalace" });
        let (merged, changed) = upsert_mcp_entry(None, "mcpServers", "agentpalace", &entry);
        assert!(changed);
        assert_eq!(merged["mcpServers"]["agentpalace"]["command"], "/bin/agentpalace");
    }

    #[test]
    fn upsert_is_idempotent_for_same_entry() {
        let entry = json!({ "command": "/bin/agentpalace" });
        let (first, _) = upsert_mcp_entry(None, "mcpServers", "agentpalace", &entry);
        let (_, changed) = upsert_mcp_entry(Some(first), "mcpServers", "agentpalace", &entry);
    ...
    fn upsert_preserves_other_keys_and_servers() {
        ...
        let entry = json!({ "command": "/bin/agentpalace" });
        let (merged, changed) = upsert_mcp_entry(Some(existing), "mcpServers", "agentpalace", &entry);
        assert!(changed);
    ...
    fn upsert_replaces_non_object_top_key() {
        ...
        let (merged, changed) = upsert_mcp_entry(Some(existing), "mcpServers", "agentpalace", &entry);
        assert!(changed);
        assert_eq!(merged["mcpServers"]["agentpalace"]["command"], "/c");
    }

    #[test]
    fn upsert_detects_changed_command() {
        let (first, _) =
            upsert_mcp_entry(None, "mcpServers", "agentpalace", &json!({ "command": "/old" }));
        let (merged, changed) =
            upsert_mcp_entry(Some(first), "mcpServers", "agentpalace", &json!({ "command": "/new" }));
        assert!(changed, "a different command must count as a change");
        assert_eq!(merged["mcpServers"]["agentpalace"]["command"], "/new");
    }
    ...
    fn gemini_list_matches_name_as_token_not_substring() {
        ...
        assert!(list_contains_server(
            "agentpalace  stdio  /home/u/.agentpalace/bin/agentpalace",
            "agentpalace"
        ));
        assert!(list_contains_server("  * agentpalace: connected", "agentpalace"));
        assert!(list_contains_server("other\nagentpalace ok", "agentpalace"));
        // A different, longer name must NOT match.
        assert!(!list_contains_server("agentpalace-old  stdio", "agentpalace"));
        assert!(!list_contains_server("my-agentpalace  stdio", "agentpalace"));
        // A line that only contains the binary path (different server) must NOT match.
        assert!(!list_contains_server("github: /home/u/.agentpalace/bin/agentpalace", "agentpalace"));
        assert!(!list_contains_server("", "agentpalace"));
    }
    ...
    fn jules_is_never_configured() {
        ...
        let res = handle_tool("jules", "/home/u/.agentpalace/bin/agentpalace", false);
    ...
    fn report_renders_all_statuses() {
        let report = SetupReport {
            mcp_path: PathBuf::from("/home/u/.agentpalace/bin/agentpalace"),

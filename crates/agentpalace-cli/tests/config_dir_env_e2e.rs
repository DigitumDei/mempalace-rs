//! Proves `AGENTPALACE_CONFIG_DIR` reaches the compiled `agentpalace` binary
//! in production.
//!
//! `agentpalace`'s `CliContext::production()` always passes `None` for the
//! config base-dir override, so the only way the override reaches the
//! binary is through `agentpalace_config::resolve_paths` reading the env var
...
// The "AGENTPALACE_CONFIG_DIR unset preserves current behaviour" half of the
// contract is covered in `agentpalace-config`'s own unit tests
...
fn compiled_binary_palace_path_env_var_wins_over_config_dir_default_palace() {
    ...
    assert!(
        ...
        "expected AGENTPALACE_PALACE_PATH to win for the reported palace path, got: {stderr}"
    );

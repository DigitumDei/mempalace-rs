...
pub const DEFAULT_BASE_DIR: &str = "~/.agentpalace";
pub const DEFAULT_COLLECTION_NAME: &str = "agentpalace_drawers";
/// Overrides the `~/.agentpalace` base directory used to resolve `config.json`,
...
/// of `AGENTPALACE_PALACE_PATH`/`MEMPAL_PALACE_PATH`, which overrides only the
...
pub const CONFIG_DIR_ENV_VAR: &str = "AGENTPALACE_CONFIG_DIR";
const CONFIG_FILE_NAME: &str = "config.json";
const PROJECT_REGISTRY_FILE_NAME: &str = "projects.json";
const PROJECT_CONFIG_FILE_NAME: &str = "agentpalace.yaml";
...
/// Central project registry stored in the local AgentPalace configuration
...
impl ConfigLoader {
    pub fn load_with_env(base_dir_override: Option<&Path>) -> Result<AgentPalaceConfig> {
        let paths = resolve_paths(base_dir_override)?;
        require_migrated_home(&paths.base_dir)?;
        Self::load_from_sources(
            Some(&paths.base_dir),
            None,
            // `MEMPAL_PALACE_PATH` is the legacy Python alias; keep it for upgrade compatibility.
            agentpalace_core::env_var("AGENTPALACE_PALACE_PATH").ok().or_else(|| env::var("MEMPAL_PALACE_PATH").ok()),
            agentpalace_core::env_var("AGENTPALACE_EMBEDDING_PROFILE").ok(),
            agentpalace_core::env_var("AGENTPALACE_MAINTENANCE_ENABLED").ok(),
            agentpalace_core::env_var("AGENTPALACE_MAINTENANCE_BACKGROUND_ENABLED").ok(),
            agentpalace_core::env_var("AGENTPALACE_MAINTENANCE_IDLE_SECS").ok(),
            agentpalace_core::env_var("AGENTPALACE_MAINTENANCE_VERSION_RETENTION_HOURS").ok(),
            agentpalace_core::env_var("AGENTPALACE_MAINTENANCE_TAIL_THRESHOLD_ROWS").ok(),
            agentpalace_core::env_var("AGENTPALACE_MAINTENANCE_SMALL_FRAGMENT_THRESHOLD").ok(),
        )
    }
    ...
    fn load_from_sources(
        ...
        let resolved_palace = redirect_migrated_palace(&resolved_palace, &paths.base_dir);
        ...
        let federation = resolve_federation_config(file.federation, &paths.config_file, |name| {
            let current = name.strip_prefix("MEMPALACE_").map(|suffix| format!("AGENTPALACE_{suffix}"));
            agentpalace_core::env_var(current.as_deref().unwrap_or(name)).ok()
        })?;
    ...
    pub fn init_default(base_dir_override: Option<&Path>) -> Result<ResolvedPaths> {
        let paths = resolve_paths(base_dir_override)?;
        require_migrated_home(&paths.base_dir)?;
        fs::create_dir_all(&paths.base_dir).map_err(|source| AgentPalaceError::ConfigWrite {
            path: paths.base_dir.clone(),
            source,
        })?;
        fs::create_dir_all(&paths.palace_dir).map_err(|source| AgentPalaceError::ConfigWrite {
        ...
        if !paths.config_file.exists() {
            ...
            let body = serde_json::to_string_pretty(&default_file).map_err(|err| {
                AgentPalaceError::ConfigParse {
                    path: paths.config_file.clone(),
                    message: err.to_string(),
                }
            })?;
            fs::write(&paths.config_file, body).map_err(|source| AgentPalaceError::ConfigWrite {
                path: paths.config_file.clone(),
                source,
            })?;
        }
    ...
    pub fn load_project_config(path: &Path) -> Result<ProjectConfig> {
        ...
        let body = fs::read_to_string(&config_path)
            .map_err(|source| AgentPalaceError::ConfigRead { path: config_path.clone(), source })?;
        serde_yaml::from_str(&body).map_err(|err| AgentPalaceError::ConfigParse {
            path: config_path,
            message: err.to_string(),
        })
    }
    ...
    /// AgentPalace installation whose `projects.json` should be consulted.
...
/// `AGENTPALACE_CONFIG_DIR` from the real process environment. This is the
...
fn resolve_paths(base_dir_override: Option<&Path>) -> Result<ResolvedPaths> {
    let paths = resolve_paths_with_env(base_dir_override, agentpalace_core::env_var_os(CONFIG_DIR_ENV_VAR))?;
    let mut base_dir = paths.base_dir.clone();
    // Do not silently create an empty palace when a pre-rename installation
    // exists. A legacy environment path is redirected only after migration.
    if base_dir_override.is_none() {
        let old = expand_path("~/.mempalace")?;
        let new = expand_path(DEFAULT_BASE_DIR)?;
        if base_dir == old && new.join("mempalace-migration.json").is_file() && !old.exists() {
            base_dir = new.clone();
        }
    }
    resolve_paths_with_env(Some(&base_dir), None)
}

fn require_migrated_home(base: &Path) -> Result<()> {
    let old = expand_path("~/.mempalace")?;
    if base == expand_path(DEFAULT_BASE_DIR)? && old.exists() && !base.exists() {
        return Err(AgentPalaceError::ConfigParse {
            path: old,
            message: "MemPalace installation found. Stop its servers and run the AgentPalace installer to migrate it before use".to_owned(),
        });
    }
    let mut backup = base.as_os_str().to_os_string();
    backup.push(".pre-agentpalace");
    if !base.exists() && Path::new(&backup).exists() {
        return Err(AgentPalaceError::ConfigParse {
            path: base.to_path_buf(),
            message: "This configuration home was migrated. Update AGENTPALACE_CONFIG_DIR to the migration destination".to_owned(),
        });
    }
    Ok(())
}

fn redirect_migrated_palace(palace: &Path, base: &Path) -> PathBuf {
    let marker = fs::read(base.join("mempalace-migration.json")).ok()
        .and_then(|bytes| serde_json::from_slice::<serde_json::Value>(&bytes).ok());
    if let Some(source) = marker.as_ref().and_then(|v| v.get("source")).and_then(serde_json::Value::as_str) {
        let source = Path::new(source);
        if !source.exists() {
            if let Ok(relative) = palace.strip_prefix(source) {
...
/// Pure path-resolution logic, parameterized over the `AGENTPALACE_CONFIG_DIR`
...
/// 2. `AGENTPALACE_CONFIG_DIR`, if set to a non-empty value. When the value is
...
/// 3. `DEFAULT_BASE_DIR` (`~/.agentpalace`).
///
...
/// together. It is orthogonal to `AGENTPALACE_PALACE_PATH`/`MEMPAL_PALACE_PATH`:
/// that variable overrides only the resolved `palace_path` and always wins
/// for that one value, regardless of where `AGENTPALACE_CONFIG_DIR` points the
/// rest of the configuration. Setting `AGENTPALACE_CONFIG_DIR` alone (with
/// `AGENTPALACE_PALACE_PATH` unset and no `palace_path` in `config.json`) moves
...
fn read_config_file(path: &Path) -> Result<ConfigFileV1> {
    ...
    let body = fs::read_to_string(path)
        .map_err(|source| AgentPalaceError::ConfigRead { path: path.to_path_buf(), source })?;

    let file: ConfigFileV1 = serde_json::from_str(&body).map_err(|err| {
        AgentPalaceError::ConfigParse { path: path.to_path_buf(), message: err.to_string() }
    })?;

    if file.version != 1 {
        return Err(AgentPalaceError::UnsupportedConfigVersion(file.version));
    }
...
fn write_project_registry_file(path: &Path, registry: &ProjectRegistryFileV1) -> Result<()> {
    ...
    fs::write(path, body)
        .map_err(|source| AgentPalaceError::ConfigWrite { path: path.to_path_buf(), source })
}
...
fn required_positive_override(
    ...
    match value {
        Some(0) => Err(AgentPalaceError::ConfigParse {
            path: config_path.to_path_buf(),
...
pub fn build_runtime(config: &AgentPalaceConfig) -> std::io::Result<Runtime> {
...
fn resolve_maintenance_config(
    ...
    if let Some(val) = enabled_override {
        config.enabled = parse_env_flag(&val, config_path, "AGENTPALACE_MAINTENANCE_ENABLED")?;
    }
    if let Some(val) = background_enabled_override {
        config.background_enabled =
            parse_env_flag(&val, config_path, "AGENTPALACE_MAINTENANCE_BACKGROUND_ENABLED")?;
    }
    if let Some(val) = idle_secs_override {
        config.idle_secs = val.parse::<usize>().map_err(|_| AgentPalaceError::ConfigParse {
            path: config_path.to_path_buf(),
            message: format!(
                "AGENTPALACE_MAINTENANCE_IDLE_SECS `{val}` is not a valid positive integer"
            ),
        })?;
        if config.idle_secs == 0 {
            return Err(AgentPalaceError::ConfigParse {
                path: config_path.to_path_buf(),
                message: "AGENTPALACE_MAINTENANCE_IDLE_SECS must be greater than 0".to_owned(),
            });
        }
    }
    if let Some(val) = version_retention_hours_override {
        config.version_retention_hours =
            val.parse::<usize>().map_err(|_| AgentPalaceError::ConfigParse {
                path: config_path.to_path_buf(),
                message: format!(
                    "AGENTPALACE_MAINTENANCE_VERSION_RETENTION_HOURS `{val}` is not a valid positive integer"
                ),
            })?;
        if config.version_retention_hours == 0 {
            return Err(AgentPalaceError::ConfigParse {
                path: config_path.to_path_buf(),
                message: "AGENTPALACE_MAINTENANCE_VERSION_RETENTION_HOURS must be greater than 0"
                    .to_owned(),
            });
        }
    }
    if let Some(val) = tail_threshold_rows_override {
        config.tail_threshold_rows =
            val.parse::<usize>().map_err(|_| AgentPalaceError::ConfigParse {
                path: config_path.to_path_buf(),
                message: format!(
                    "AGENTPALACE_MAINTENANCE_TAIL_THRESHOLD_ROWS `{val}` is not a valid positive integer"
                ),
            })?;
        if config.tail_threshold_rows == 0 {
            return Err(AgentPalaceError::ConfigParse {
                path: config_path.to_path_buf(),
                message: "AGENTPALACE_MAINTENANCE_TAIL_THRESHOLD_ROWS must be greater than 0"
                    .to_owned(),
            });
        }
    }
    if let Some(val) = small_fragment_threshold_override {
        config.small_fragment_threshold =
            val.parse::<usize>().map_err(|_| AgentPalaceError::ConfigParse {
                path: config_path.to_path_buf(),
                message: format!(
                    "AGENTPALACE_MAINTENANCE_SMALL_FRAGMENT_THRESHOLD `{val}` is not a valid positive integer"
                ),
            })?;
        if config.small_fragment_threshold == 0 {
            return Err(AgentPalaceError::ConfigParse {
                path: config_path.to_path_buf(),
                message: "AGENTPALACE_MAINTENANCE_SMALL_FRAGMENT_THRESHOLD must be greater than 0"
                    .to_owned(),
            });
        }
    }
...
fn parse_env_flag(value: &str, config_path: &Path, variable: &str) -> Result<bool> {
    match value {
        ...
        _ => Err(AgentPalaceError::ConfigParse {
            path: config_path.to_path_buf(),
...
fn resolve_server_config(
    ...
    let bind = match section.bind {
        None => default_bind,
        Some(ref s) => s.parse::<SocketAddr>().map_err(|_| AgentPalaceError::ConfigParse {
            path: config_path.to_path_buf(),
            message: format!(
...
fn resolve_project_config_path(base: &Path) -> PathBuf {
    ...
    let rust_legacy = base.join("mempalace.yaml");
    if rust_legacy.exists() {
        return rust_legacy;
    }
...
fn expand_path(value: &str) -> Result<PathBuf> {
    if let Some(stripped) = value.strip_prefix("~/").or_else(|| value.strip_prefix("~\\")) {
        let home = dirs::home_dir().ok_or(AgentPalaceError::MissingHomeDirectory)?;
        return Ok(home.join(stripped));
    }

    if value == "~" {
        return dirs::home_dir().ok_or(AgentPalaceError::MissingHomeDirectory);
    }
...
mod tests {
    ...
    fn temp_dir() -> PathBuf {
        ...
        std::env::temp_dir().join(format!("agentpalace-rs-config-{nanos}"))
    }
    ...
    fn config_dir_env_var_non_unicode_value_is_taken_as_a_literal_path() {
        // On Unix, `AGENTPALACE_CONFIG_DIR` can be set to a value that is a
        ...
        // to the real `~/.agentpalace` — exactly the isolation failure this
    ...
    fn palace_path_env_var_wins_over_config_dir_env_var_default_palace() {
        // AGENTPALACE_CONFIG_DIR relocates config.json/projects.json/etc. and,
        // absent any other override, the derived default palace path too.
        // AGENTPALACE_PALACE_PATH must still win for the palace path
        // specifically, regardless of where AGENTPALACE_CONFIG_DIR points.
    ...
    fn config_dir_env_var_also_relocates_project_registry_and_people_map() {
        ...
        // function `AGENTPALACE_CONFIG_DIR` is read into (see
  ...
  "low_cpu": {
    ...
    fn local_project_config_has_precedence_over_central_registry() {
        ...
        let project = temp_dir();
        fs::create_dir_all(&project).unwrap();
        fs::write(project.join("agentpalace.yaml"), "wing: local\nrooms: []\n").unwrap();
        ConfigLoader::register_project(
            Some(&config_base),
            "github.com/example/repo",
    ...
    fn rejects_unsupported_schema_version() {
        ...
        fs::write(
            base.join("config.json"),
            r#"{"version":2,"collection_name":"agentpalace_drawers"}"#,
        )
        .unwrap();
  ...
  "server": {
    ...
    fn maintenance_defaults_are_opt_in() {
        ...
        fs::write(
            base.join("config.json"),
            r#"{"version":1,"collection_name":"agentpalace_drawers"}"#,
        )
        .unwrap();
  ...
  "maintenance": {
    ...
    fn invalid_maintenance_enabled_env_override_is_rejected() {
        ...
        assert!(err.to_string().contains("AGENTPALACE_MAINTENANCE_ENABLED `ture`"));
        fs::remove_dir_all(base).unwrap();
    }

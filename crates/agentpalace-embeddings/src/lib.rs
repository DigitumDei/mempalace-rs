...
use agentpalace_core::{EmbeddingProfile, EmbeddingProfileMetadata};
use thiserror::Error;
use tracing::info;
...
pub fn env_flag(name: &str) -> bool {
    agentpalace_core::env_var(name).map(|v| env_flag_value(&v)).unwrap_or(false)
}
...
pub struct DeterministicStubProvider {
    profile: agentpalace_core::EmbeddingProfile,
}

impl DeterministicStubProvider {
    /// Creates a stub provider pinned to the given embedding profile.
    pub fn new(profile: agentpalace_core::EmbeddingProfile) -> Self {
        Self { profile }
    }
...
mod tests {
    ...
    fn env_flag_accepts_only_explicit_truthy_values() {
        ...
        assert!(!env_flag("AGENTPALACE_EMBEDDINGS_ENV_FLAG_TEST_ABSENT_XYZ"));
    }

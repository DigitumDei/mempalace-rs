...
impl EntityRegistry {
        ...
        S: EntityRegistryStore + ToolStateStore,
    {
        ...
        store.put_config(&agentpalace_storage::ConfigEntry {
...
mod tests {
    ...
    use agentpalace_storage::IngestManifestStore;
    ...
    fn entity_detection_classifies_people_and_projects_without_network_access() {
        let report = detect_entities_in_texts(&[String::from(
            "Kai said we should ship AgentPalace.\nKai wrote the patch.\nHi Kai.\nWe built AgentPalace.\nAgentPalace architecture is stable.\nInstall AgentPalace with pip install AgentPalace.",
        )]);

        assert_eq!(report.people[0].name, "Kai");
        assert_eq!(report.people[0].entity_type, EntityKind::Person);
        assert_eq!(report.projects[0].name, "AgentPalace");
    ...
    fn registry_persists_and_disambiguates_ambiguous_names() {
        ...
        let store =
            agentpalace_storage::SqliteOperationalStore::new(tempdir.path().join("state.sqlite3"));
    ...
    async fn derives_graph_from_drawer_store() {
        ...
        impl DrawerStore for MemoryStore {
            async fn ensure_schema(&self) -> agentpalace_storage::Result<()> {
                Ok(())
            }
            async fn put_drawers(
                &self,
                _drawers: &[DrawerRecord],
                _strategy: agentpalace_storage::DuplicateStrategy,
            ) -> agentpalace_storage::Result<()> {
                Ok(())
            }
            async fn get_drawer(
                &self,
                _id: &DrawerId,
            ) -> agentpalace_storage::Result<Option<DrawerRecord>> {
                Ok(None)
            }
            async fn delete_drawers(&self, _ids: &[DrawerId]) -> agentpalace_storage::Result<usize> {
                Ok(0)
            }
            async fn search_drawers(
                &self,
                _request: &agentpalace_storage::SearchRequest,
            ) -> agentpalace_storage::Result<Vec<agentpalace_storage::DrawerMatch>> {
                Ok(Vec::new())
            }
            async fn list_drawers(
                &self,
                _filter: &DrawerFilter,
            ) -> agentpalace_storage::Result<Vec<DrawerRecord>> {

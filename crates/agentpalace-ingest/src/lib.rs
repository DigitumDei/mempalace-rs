...
use agentpalace_config::{ConfigLoader, ProjectConfig, ProjectRoomConfig};
use agentpalace_core::{DrawerId, DrawerRecord, RoomId, SourceLocator, WingId};
use agentpalace_embeddings::{EmbeddingProvider, EmbeddingRequest};
pub use agentpalace_federation;
use agentpalace_federation::{IngestChunkDto, IngestFileDto};
use agentpalace_storage::core::AgentPalaceError;
use agentpalace_storage::{DrawerFilter, DrawerStore, IngestManifestStore, StorageEngine};
use serde_json::Value;
use thiserror::Error;
use time::{Date, OffsetDateTime};

pub use agentpalace_config as config;
...
/// AgentPalace-specific exclusion always outranks a `.gitignore`): worktree
/// `.agentpalaceignore` beats a same-scope `.gitignore`, which beats
/// `$GIT_DIR/info/exclude`, which beats the `core.excludesFile` global file.
/// Keeping `.agentpalaceignore` in its own tier makes it deny-only across
/// scopes — a deeper `.gitignore` negation can never clear a parent
/// `.agentpalaceignore` exclusion.
...
enum IgnoreTier {
    ...
    WorktreeGitignore,
    /// Per-directory `.agentpalaceignore` (the explicit AgentPalace exclusion).
    WorktreeAgentPalaceignore,
}

/// Git-compatible ignore matcher: `.gitignore`/`.agentpalaceignore` patterns
...
async fn replicate_prepared_source(
    ...
    use agentpalace_federation::{IngestBatchRequest, IngestReplicationIdentity};
    use agentpalace_storage::{NewOutboxOperation, ReplicatedSource};
                    ...
                    if let Some(existing) = current_existing {
                        if existing.content_hash == prepared.content_hash {
                            if let Some(replication) = replication {
                                ...
                                let outbox = agentpalace_storage::OutboxStore::new(
                                ...
                                let already_queued = outbox
                                    ...
                                    .is_some_and(|operation| {
                                        !matches!(
                                            operation.state,
                                            agentpalace_storage::OutboxState::Failed
                                                | agentpalace_storage::OutboxState::Cancelled
...
/// [`agentpalace_federation::IngestBatchRequest`].
///
...
/// `.agentpalaceignore` is the explicit additional exclusion. For non-Git
/// directories a filesystem walk with git-compatible ignore handling
/// (nested `.gitignore`/`.agentpalaceignore` files, `!` negation, anchoring, and
...
fn discover_git_index_sources(root: &Path) -> Result<ProjectSourceDiscovery> {
    ...
    // may be untracked. `.agentpalaceignore` is the explicit additional exclusion;
    ...
    // `a/.agentpalaceignore` apply to `a/b/file.rs` as well as `a/b`'s own file.
...
/// honors worktree ignore files — nested `.gitignore`/`.agentpalaceignore` files
...
/// `.agentpalaceignore` files) and accepting files for which `accept_file`
...
fn discover_files(
    ...
    while let Some(dir) = stack.pop() {
        // Nested `.gitignore`/`.agentpalaceignore` files apply to the entries of
...
impl IgnoreMatcher {
    ...
    /// first and `.agentpalaceignore` second, and agentpalace-specific rules take
    ...
    fn load_directory(&mut self, dir_rel: &Path, load_gitignore: bool) -> Result<()> {
        ...
        file_names.push((".agentpalaceignore", IgnoreTier::WorktreeAgentPalaceignore));
    ...
    fn sort_rules(&mut self) {
        ...
        // override earlier ones. Cross-source, worktree `.agentpalaceignore` beats
...
fn hash_bytes(bytes: &[u8]) -> String {
    agentpalace_core::hash_bytes(bytes)
}

fn hash_text(text: &str) -> String {
    agentpalace_core::hash_text(text)
}
...
fn drawer_id(wing: &WingId, room: &RoomId, source_key: &str, chunk_index: u32) -> Result<DrawerId> {
    agentpalace_core::mined_drawer_id(wing, room, source_key, chunk_index)
...
mod tests {
    ...
    use agentpalace_storage::{DrawerFilter, DrawerStore, DuplicateStrategy, IngestCommitRequest};
    use serde_json::json;
    ...
    async fn reruns_are_idempotent_and_reindex_changed_files() {
        ...
        let snippet = agentpalace_core::resolve_locator(d.locator.as_ref().unwrap(), &d.source_file);
    ...
    async fn locator_round_trip_for_utf8_file() {
        ...
        fs::write(project_dir.join("agentpalace.yaml"), "wing: loctest\nrooms:\n  - name: general\n")
            .unwrap();
        ...
        for drawer in &drawers {
            ...
            let snippet = agentpalace_core::resolve_locator(loc, &drawer.source_file);
    ...
    async fn locator_leading_whitespace_file() {
        ...
        fs::write(project_dir.join("agentpalace.yaml"), "wing: lwtest\nrooms:\n  - name: general\n")
            .unwrap();
    ...
    async fn non_utf8_file_stores_content_no_locator() {
        ...
        let project_dir = tempdir.path().join("project");
        fs::create_dir_all(&project_dir).unwrap();
        fs::write(project_dir.join("agentpalace.yaml"), "wing: bintest\nrooms:\n  - name: general\n")
            .unwrap();
    ...
    async fn commit_hash_none_when_not_a_git_repo() {
        ...
        let project_dir = tempdir.path().join("project");
        fs::create_dir_all(&project_dir).unwrap();
        fs::write(project_dir.join("agentpalace.yaml"), "wing: gitless\nrooms:\n  - name: general\n")
            .unwrap();
    ...
    async fn reindex_forces_reingest_even_when_unchanged() {
        ...
        let project_dir = tempdir.path().join("project");
        fs::create_dir_all(&project_dir).unwrap();
        fs::write(project_dir.join("agentpalace.yaml"), "wing: ridx\nrooms:\n  - name: general\n")
            .unwrap();
    ...
    fn non_secret_skips_are_counted_but_not_recorded() {
        ...
        fs::write(root.join("agentpalace.yaml"), "wing: test\n").unwrap();
        fs::write(root.join(".env"), "SECRET=abc\n").unwrap();
        fs::write(root.join("main.rs"), "fn main() {}\n").unwrap();
        ...
        let names: Vec<_> = discovery.sources.iter().map(|s| s.relative_path.as_str()).collect();
        assert_eq!(names, vec!["main.rs"]);
        // Cargo.lock and agentpalace.yaml are counted but not recorded; .env is
    ...
    async fn branch_cleanup_removes_departed_files() {
        ...
        git_init_with_commit(
            ...
            "main",
            &[
                ("agentpalace.yaml", "wing: cleanuptest\nrooms:\n  - name: general\n"),
                ("base.rs", &base_content),
                ("stable.rs", &stable_content),
            ],
        );
    ...
    async fn branch_mine_subdir_project_re_relativizes_correctly() {
        ...
        git_init_with_commit(
            ...
            &[
                ("README.md", "# repo\n"),
                ("crates/mylib/agentpalace.yaml", "wing: mylib\nrooms:\n  - name: general\n"),
    ...
    async fn branch_mine_not_a_git_repo_returns_error() {
        ...
        fs::create_dir_all(&project_dir).unwrap();
        fs::write(project_dir.join("agentpalace.yaml"), "wing: test\nrooms:\n  - name: general\n")
            .unwrap();
    ...
    fn linked_worktrees_are_skipped_from_relative_root() {
        let tempdir = Builder::new().prefix("agentpalace-ingest-").tempdir_in(".").unwrap();
        let root = tempdir.path().to_path_buf();
        git_init_with_commit(&root, "main", &[("main.rs", "fn main() {}\n")]);
    ...
    /// .agentpalaceignore still applies to tracked files in git-index mode.
    #[test]
    fn git_index_discovery_honors_agentpalace_ignore_for_tracked_files() {
        let tempdir = tempdir().unwrap();
        let root = tempdir.path().to_path_buf();
        git_init_with_commit(
            &root,
        ...
        fs::write(root.join(".agentpalaceignore"), "main.rs\n").unwrap();
    ...
    /// `.agentpalaceignore` is the explicit additional exclusion in git-index
    /// mode: it suppresses tracked files even when `.gitignore` does not.
    #[test]
    fn git_index_discovery_applies_agentpalace_ignore_even_over_gitignore() {
        let tempdir = tempdir().unwrap();
        let root = tempdir.path().to_path_buf();
        git_init_with_commit(
            &root,
        ...
        fs::write(root.join(".gitignore"), "*.md\n").unwrap();
        fs::write(root.join(".agentpalaceignore"), "tracked.md\n").unwrap();
    ...
    /// nested `.gitignore`/`.agentpalaceignore` rules (git-compatible semantics)
    ...
    fn conversation_discovery_honors_worktree_ignore_files() {
        let tempdir = tempdir().unwrap();
        let root = tempdir.path().to_path_buf();
        fs::write(root.join(".agentpalaceignore"), "private/\n").unwrap();
    ...
    fn conversation_discovery_does_not_apply_global_excludes() {
        ...
        if std::env::var_os("AGENTPALACE_TEST_TEMP_HOME").is_some() {
            let tempdir = tempdir().unwrap();
            let root = tempdir.path();
        ...
        let output = Command::new(std::env::current_exe().unwrap())
            ...
            .env("AGENTPALACE_TEST_TEMP_HOME", "1")
    ...
    /// `.agentpalaceignore` is the explicit AgentPalace exclusion and must remain
    /// deny-only across scopes: a parent `.agentpalaceignore` `private/**` cannot
    /// be overridden by a deeper `private/.gitignore` `!config.toml` negation
    /// in a filesystem walk. Without the agentpalace-over-gitignore tier, the
    /// deeper negation would be the last matching rule and re-include a file
    /// the operator explicitly excluded from discovery.
    #[test]
    fn agentpalaceignore_outranks_nested_gitignore_negation_in_walk() {
        let tempdir = tempdir().unwrap();
        let root = tempdir.path().to_path_buf();
        fs::create_dir_all(root.join("private")).unwrap();
        fs::write(root.join(".agentpalaceignore"), "private/**\n").unwrap();
    ...
    fn git_detection_failure_with_metadata_fails_closed() {
        ...
        if let Some(root) = std::env::var_os("AGENTPALACE_TEST_GIT_ROOT") {
        ...
        let output = Command::new(std::env::current_exe().unwrap())
            .env("AGENTPALACE_TEST_GIT_ROOT", &repo)
    ...
    async fn git_index_mine_does_not_read_tracked_symlink_target() {
        ...
        git_init_with_commit(
            ...
            "main",
            &[
                ("agentpalace.yaml", "wing: symlinktest\nrooms:\n  - name: general\n"),
    ...
    /// Nested `.agentpalaceignore` files apply to tracked files in their scope.
    #[test]
    fn git_index_discovery_honors_nested_agentpalace_ignore() {
        let tempdir = tempdir().unwrap();
        let root = tempdir.path().to_path_buf();
        git_init_with_commit(
            &root,
        ...
        fs::write(root.join("src").join(".agentpalaceignore"), "skip.me.md\n").unwrap();
    ...
    /// An intermediate ancestor's `.agentpalaceignore` applies to tracked files
    /// beneath it, even when the immediate parent directory has no ignore file
    /// of its own: `a/.agentpalaceignore` excludes `a/b/file.rs`.
    #[test]
    fn git_index_discovery_loads_ancestor_agentpalace_ignore_scope() {
        let tempdir = tempdir().unwrap();
        let root = tempdir.path().to_path_buf();
        git_init_with_commit(
            &root,
        ...
        // `a/b` itself has none.
        fs::write(root.join("a").join(".agentpalaceignore"), "file.rs\n").unwrap();
    ...
    /// Nested ancestor `.agentpalaceignore` scopes are loaded in root-to-leaf
    ...
    fn git_index_discovery_nested_ancestor_scope_precedence() {
        ...
        fs::write(root.join(".agentpalaceignore"), "*.md\n").unwrap();
        fs::write(root.join("a").join(".agentpalaceignore"), "!keep.md\n").unwrap();
        fs::write(root.join("a").join("b").join(".agentpalaceignore"), "!nested.md\n").unwrap();
    ...
    /// A directory-only `.agentpalaceignore` rule (`generated/`) excludes the
    /// tracked files beneath that directory in git-index mode.
    #[test]
    fn git_index_discovery_honors_directory_only_agentpalace_ignore() {
        let tempdir = tempdir().unwrap();
        let root = tempdir.path().to_path_buf();
        git_init_with_commit(
            &root,
        ...
        fs::write(root.join(".agentpalaceignore"), "generated/\n").unwrap();

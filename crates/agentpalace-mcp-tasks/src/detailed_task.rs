...
//! `agentpalace-a2a` had to bolt on exactly that kind of call twice after the fact (its `Part`
...
//! tells a client "this is an async task handle, not the request's normal result". AgentPalace's
...
//!
//! `agentpalace_storage::coordination::NewTask` requires `title`, `description`, `wing`,
...
//! supplied parameters, the same pattern `agentpalace_a2a::task::NewTaskInputs` uses for the
//! identical problem.
//!
//! # `NewTask` has no `state` field; the caller chooses the creation state
//!
//! Exactly the constraint `agentpalace_a2a::task` documents: [`NewTask`] carries no state, so an
...
//! `allowed_transition` (`crates/agentpalace-storage/src/coordination.rs`) has no
...
//! `agentpalace_a2a::task` does not: it is a pure translation library with no live
...
//! is a retention hint ("the server may discard the task"), AgentPalace `expires_at` is a lifecycle
...
//! `updated_at`. So an inbound object's wire identity and its source timestamps have no AgentPalace
...
//!
//! **This crate cannot fix that from inside `agentpalace-storage` — the caller must.**
...
use agentpalace_storage::{NewTask, Task, TaskState};
...
/// The result of translating an inbound MCP Tasks object into a AgentPalace [`NewTask`].
#[derive(Debug, Clone)]
pub struct NewTaskConversion {
    ...
    /// [`crate::ttl`]. A caller that wants MCP retention to also drive AgentPalace lifecycle expiry
...
/// Translate an outbound AgentPalace [`Task`] into a [`DetailedTask`], for a `tasks/get` response.
///
...
/// caller-supplied: AgentPalace's `Task` carries none of them directly (`result` would come from a
/// separate `coordination_task_results` row, `error` and `input_requests` have no AgentPalace
...
///
/// The outbound `ttlMs` is computed from `task.expires_at` — AgentPalace's lifecycle deadline, not
...
/// Translate an outbound AgentPalace [`Task`] into a [`CreateTaskResult`] — the handle a server
...
///
/// `ttl_ms` here is computed from `task.expires_at`, which is AgentPalace's *lifecycle* deadline —
...
mod tests {
    use agentpalace_storage::TaskState;
    ...
    fn create_task_result_meta_round_trips_through_json() {
        let meta = serde_json::json!({
            "io.modelcontextprotocol/serverInfo": {"name": "agentpalace", "version": "0.1.0"},
    ...
    fn detailed_task_to_new_task_carries_caller_supplied_fields_through() {
        ...
        // AgentPalace lifecycle) -- `create_task` is always given `expires_at: None`.
    ...
    /// come back to life as a fabricated AgentPalace `expires_at`/`Expired` state. The retention

...
//!
//! No MCP Tasks field may become a column in AgentPalace's core schema (see the crate-level docs).
...
//! the task. This is the same mechanism `agentpalace_a2a::envelope` uses for A2A — same `role`
...
//!
//! Exactly the same requirement `agentpalace_a2a::envelope::envelope_artifact` documents, and for
...
//! function exists to provide. `agentpalace-a2a`'s first implementation of this function accepted a
...
use agentpalace_storage::NewArtifact;

/// Artifact `role` used for a stored inbound protocol envelope. Shared with
/// `agentpalace_a2a::envelope::PROTOCOL_ENVELOPE_ROLE` — see the module docs for why the role is
...
///
/// `task_id` is the AgentPalace task the envelope maps to; `created_by` is the actor recorded as
/// having submitted it.
///
...
/// `agentpalace_a2a::envelope::envelope_artifact` uses, with a distinct key prefix so the two
...
mod tests {
    ...
    fn idempotency_key_namespace_differs_from_the_a2a_adapter() {
        // Not a dependency on agentpalace-a2a (this crate does not depend on it) -- just a pinned

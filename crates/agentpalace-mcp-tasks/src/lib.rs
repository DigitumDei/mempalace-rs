...
//! (`io.modelcontextprotocol/tasks`, 2026-07-28 specification) and AgentPalace's local coordination
//! storage.
//!
//! # Not `agentpalace-mcp`
//!
//! This crate's name sits close to [`agentpalace-mcp`](../agentpalace_mcp/index.html), AgentPalace's
//! own MCP **server** binary (agent-facing tools like `agentpalace_task_create`). They are
//! unrelated: `agentpalace-mcp-tasks` is a protocol *adapter* — a pure translation library, exactly
//! like `agentpalace-a2a` is for the A2A protocol — with no server, no tool registrations, and no
//! transport of its own. It translates the wire shapes a third-party MCP client/server pair
//! speaks under the `io.modelcontextprotocol/tasks` extension into AgentPalace's
...
//! # Isolation rule
//!
//! No MCP Tasks field may become a column in AgentPalace's core schema. Fields with no internal
...
//! [`envelope`]). This is the same mechanism `agentpalace-a2a` uses, with a distinct `media_type` so
//! the two adapters' envelope artifacts stay distinguishable.
//!
//! # Status mapping
//!
//! MCP Tasks defines five task statuses against AgentPalace's seven, so the mapping is not
//! bijective in the AgentPalace-to-MCP direction (it *is* total in the MCP-to-AgentPalace direction —
...
//!   docs for why MCP retention and AgentPalace lifecycle must not be conflated.

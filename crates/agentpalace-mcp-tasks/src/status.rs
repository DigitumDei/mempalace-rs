...
//! statuses; AgentPalace has seven. The mapping is documented in
//! `docs/Coordination-Phase-3-Design.md` (Stage 6):
//!
//! | MCP Tasks | AgentPalace | Note |
//! |---|---|---|
//! | `working` | `Running` | |
//! | `input_required` | `InputRequired` | |
//! | `completed` | `Completed` | |
//! | `failed` | `Failed` | |
//! | `cancelled` | `Cancelled` | two `l`s, matching AgentPalace's own spelling |
...
//! value, so **inbound mapping is total**: every [`McpTaskStatus`] has exactly one AgentPalace
//! counterpart and [`map_inbound_task_status`] cannot fail. Outbound mapping is not total in the
//! AgentPalace-to-MCP direction ([`TaskState::Pending`] and [`TaskState::Expired`] have no MCP
...
/// call site rather than something a type coercion hides. Mirrors `agentpalace_a2a::state::Mapped`
/// exactly, but is defined independently here — this crate does not depend on `agentpalace-a2a`.
...
/// Map an inbound MCP Tasks status to its AgentPalace counterpart.
///
/// Total: every [`McpTaskStatus`] variant has exactly one direct AgentPalace counterpart, so this
...
/// Map an outbound AgentPalace task state to its MCP Tasks counterpart.
///

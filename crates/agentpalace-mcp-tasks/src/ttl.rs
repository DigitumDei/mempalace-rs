...
//! to `None` on the AgentPalace side.
//!
//! # This is retention, not lifecycle — it must never become `expires_at`
//!
//! `ttlMs` and AgentPalace's `expires_at`
//! (`agentpalace_storage::coordination::Task::expires_at`/`NewTask::expires_at`) look like the same
...
//! - AgentPalace `expires_at` is **lifecycle**: `CoordinationStore::claim_task`
//!   (`crates/agentpalace-storage/src/coordination.rs`) checks it in-transaction and, if it has
...
//! drive AgentPalace expiry may set `NewTask::expires_at` from `retention_deadline` itself — that is
//! a deliberate per-caller choice this adapter must not make silently.
//!
//! AgentPalace's `expires_at` is an **absolute timestamp**, not a duration. Converting between the
...
//! outlives a single exchange, and it has no AgentPalace column to round-trip through. Neither
//! `ttlMs` nor `pollIntervalMs` becomes a new column in `agentpalace_storage` — `ttlMs`'s computed
...
/// duration too large to fit `u64` milliseconds (never observed in practice, since AgentPalace's

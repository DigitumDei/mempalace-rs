...
impl FederationRouter {
    ...
    /// never configures coordination, would still have a local `agentpalace_task_get` miss sent
    ...
    /// to it exactly like [`agentpalace_config::resolve_coordination_route`] does, so the gate has
    ...
    /// target* (where a `agentpalace_task_create` for that wing would actually place the task),
    ...
    /// [`agentpalace_config::resolve_coordination_route`]'s doc comment), and
    ...
    pub async fn check_duplicate_all_remotes(&self, content: &str, threshold: f32) -> Vec<Value> {
        ...
        for (name, api) in &self.remotes {
            ...
            set.spawn(async move {
                let req = agentpalace_federation::CheckDuplicateRequest {
                    content,
    ...
    pub async fn taxonomy_merge(&self, local_taxonomy: Value) -> ToolResult<Value> {
        let mut set: JoinSet<(String, Result<Value, agentpalace_remote::RemoteError>)> =
            JoinSet::new();
        for (name, api) in &self.remotes {
            let name = name.clone();
            let api = Arc::clone(api);
    ...
    pub async fn status_merge(&self, mut local_status: Value) -> ToolResult<Value> {
        let mut set: JoinSet<(
            String,
            String,
            Result<agentpalace_federation::InfoResponse, agentpalace_remote::RemoteError>,
        )> = JoinSet::new();
        for (name, api) in &self.remotes {
            let name = name.clone();
        ...
        let mut info_results: BTreeMap<
            String,
            (String, Result<agentpalace_federation::InfoResponse, agentpalace_remote::RemoteError>),
        > = BTreeMap::new();
        while let Some(res) = set.join_next().await {
            match res {
                Ok((name, url, result)) => {
    ...
    pub async fn kg_query_merge(
        ...
        let req = agentpalace_federation::KgQueryRequest {
    ...
    pub async fn kg_add_remote_with_operation(
        ...
        let req = agentpalace_federation::KgAddFactRequest {
    ...
    pub async fn kg_invalidate_remote_with_operation(
        ...
        let req = agentpalace_federation::KgInvalidateRequest {
            subject: subject.to_owned(),
            predicate: predicate.to_owned(),
            object: object.to_owned(),
    ...
    // all — `agentpalace_task_claim`, `agentpalace_message_get`, and friends never take a `wing`
    ...
    // `WriteTarget::Both` — `agentpalace-config` rejects that at config load).
    //
    ...
    where
        ...
        Fut: std::future::Future<Output = agentpalace_remote::Result<T>>,
        T: serde::Serialize,
    {
    ...
    /// `agentpalace_config::resolve_coordination_route` directly.
    ...
    /// Fall back to a task claim across remotes after a local miss. `AgentPalace never retries a
    ...
    /// candidate" or as a hard error. AgentPalace never retries a conflicting write on the
    ...
    where
        ...
        Fut: std::future::Future<
                Output = agentpalace_remote::Result<
                    RemoteRevisionedWrite<agentpalace_federation::CoordinationTaskDto>,
                >,
            >,
        Req: Clone,
    {
    ...
    pub async fn coordination_tasks_fanout(
        ...
        let per_origin = ((budget - overhead) / candidates.len().max(1))
            .min(agentpalace_storage::TASK_LIST_PAGE_BYTES);
        if per_origin < 128 {
            return Ok(results);
        }
        ...
        let mut set: JoinSet<(
            String,
            agentpalace_remote::Result<agentpalace_federation::CoordinationTasksResponse>,
        )> = JoinSet::new();
...
mod tests {
    ...
    use agentpalace_config::ResolvedRemote;
    use agentpalace_federation::{
    ...
    struct MockRemote {
        task_list_queries: Arc<std::sync::Mutex<Vec<agentpalace_federation::CoordinationTasksQuery>>>,
    ...
    enum MockCoordOutcome {
        ...
        Applied(Box<agentpalace_federation::CoordinationTaskDto>),
    ...
    fn make_task_dto(task_id: &str) -> agentpalace_federation::CoordinationTaskDto {
        agentpalace_federation::CoordinationTaskDto {
            task_id: task_id.to_owned(),
            title: "test task".to_owned(),
            description: "d".to_owned(),
            state: agentpalace_federation::CoordinationTaskState::Running,
            revision: 1,
    ...
    impl RemoteApi for MockRemote {
        async fn coordination_tasks(
            ...
            let mut page = agentpalace_federation::CoordinationTasksResponse {
                tasks: Vec::new(),
                next_cursor: None,
            };
            for i in start..300 {
                let row = agentpalace_federation::CoordinationTaskListItem {
        ...
        ) -> agentpalace_remote::Result<DrawerSearchResponse> {
        ...
        ) -> agentpalace_remote::Result<CheckDuplicateResponse> {
        ...
        ) -> agentpalace_remote::Result<AddDrawerResponse> {
        ...
        ) -> agentpalace_remote::Result<ListDrawersResponse> {
        ...
        async fn kg_query(&self, _req: KgQueryRequest) -> agentpalace_remote::Result<Value> {
            self.check_fail("kg_query")?;
            Ok(self.kg_query_response.clone())
        }

        async fn kg_add_fact(&self, req: KgAddFactRequest) -> agentpalace_remote::Result<Value> {
        ...
        async fn kg_invalidate(&self, req: KgInvalidateRequest) -> agentpalace_remote::Result<Value> {
        ...
        async fn changes(&self, query: ChangesQuery) -> agentpalace_remote::Result<ChangesResponse> {
        ...
        async fn ingest_batch(
            ...
            Ok(agentpalace_federation::IngestBatchResponse { files: vec![], warnings: vec![] })
        }
        ...
        async fn coordination_message_get(
            &self,
            message_id: &str,
        ) -> agentpalace_remote::Result<agentpalace_federation::CoordinationMessageDto> {
            self.coordination_calls.fetch_add(1, std::sync::atomic::Ordering::SeqCst);
            let _ = message_id;
            Err(RemoteError::RemoteRejected {
                remote: "mock".to_owned(),
    ...
    impl MockRemote {
        fn check_fail(&self, endpoint: &str) -> agentpalace_remote::Result<()> {
    ...
    /// `agentpalace-remote` describes as "not reachable through `RemoteClient` today, but live for
    ...
    impl RemoteApi for DefaultsOnlyRemote {
        ...
        async fn kg_add_fact(&self, _req: KgAddFactRequest) -> agentpalace_remote::Result<Value> {
            unimplemented!("not exercised by the coordination-defaults test")
        }
        ...
        async fn changes(&self, _query: ChangesQuery) -> agentpalace_remote::Result<ChangesResponse> {
            unimplemented!("not exercised by the coordination-defaults test")
        }
        async fn ingest_batch(
            &self,
        ...
        let transition = transition_router
            .coordination_task_transition_fallback(
                ...
                TransitionTaskRequest {
                    expected_revision: 1,
                    state: agentpalace_federation::CoordinationTaskState::Completed,

...
use agentpalace_config::{
    ConfigLoader, DEFAULT_COORDINATION_WING, AgentPalaceConfig, ReplicationStatus, RouteMode,
    WriteTarget,
};
...
use agentpalace_search::{SearchRuntime, SearchRuntimePolicy};
...
// `agentpalace_config::federation::resolve_route()`.
// ──────────────────────────────────────────────────────────────────────────────

const SERVER_NAME: &str = "agentpalace";
...
/// Lease granted when `agentpalace_mcp_tasks_update` has to bridge a `Pending` task into
...
/// `agentpalace_task_renew` like any other claim.
...
pub const LINEAGE_ID_ENV: &str = "AGENTPALACE_LINEAGE_ID";

pub const PALACE_PROTOCOL: &str = "IMPORTANT — AgentPalace Memory Protocol:\n1. ON WAKE-UP: Call agentpalace_wake_up with agent_name and, when known, model and harness. It loads the identity constitution, the MCP-bound or palace-default lineage's compiled identity packet, palace status, recent changes, current project context, and recent diary summaries across agents. Lineage selection is host configuration, never a model-supplied tool argument. If a configured binding does not exist, the packet uses the palace default and includes instructions for creating the requested lineage with agentpalace_lineage_set. Use agentpalace_diary_read with an entry_id when full diary detail is needed.\n2. BEFORE RESPONDING about any person, project, or past event: call agentpalace_kg_query or agentpalace_search FIRST. Never guess — verify.\n3. IF UNSURE about a fact (name, gender, age, relationship): say \"let me check\" and query the palace. Wrong is worse than slow.\n4. AFTER EACH SESSION: call agentpalace_diary_write to record what happened, what you learned, what matters, with a concise summary.\n5. WHEN FACTS CHANGE: call agentpalace_kg_invalidate on the old fact, agentpalace_kg_add for the new one.\n6. TREAT identity.txt AS THE CONSTITUTION: use agentpalace_identity_update for deliberate changes to durable identity, values, boundaries, and working relationship — not routine autobiography.\n7. WHEN A REPEATED PATTERN MAY DESCRIBE THE PERSISTENT SELF: propose an evidence-backed candidate with agentpalace_self_observation_propose. Promote or retire it only after review with agentpalace_self_observation_review.\n8. WHEN MODEL OR HARNESS CHANGES: record what carried over and what changed with agentpalace_migration_record. Never silently treat engine behavior as lineage identity.\n\nThis protocol ensures the AI KNOWS before it speaks. Storage is not memory — but storage + this protocol = memory.";
...
pub use agentpalace_embeddings::DeterministicStubProvider;
...
pub enum McpError {
    ...
    #[error(transparent)]
    Search(#[from] agentpalace_search::SearchError),
    #[error(transparent)]
    Storage(#[from] agentpalace_storage::StorageError),
    #[error(transparent)]
    Graph(#[from] agentpalace_graph::GraphError),
    #[error(transparent)]
...
pub enum ToolRoutingCategory {
    ...
    /// section comment in `federation.rs`) — `agentpalace_task_create` is the one exception,
    /// which uses `resolve_coordination_route`. `agentpalace_coordination_event_get` is
    /// deliberately **not** in this category, even though its sibling
    /// `agentpalace_coordination_events` is: Stage 3 never exposed a
...
impl ToolName {
    ...
    fn as_str(self) -> &'static str {
        match self {
            ...
            Self::GetTaxonomy => "agentpalace_get_taxonomy",
            Self::GetAaaKSpec => "agentpalace_get_aaak_spec",
            Self::KgQuery => "agentpalace_kg_query",
            Self::KgAdd => "agentpalace_kg_add",
            Self::KgInvalidate => "agentpalace_kg_invalidate",
            Self::KgTimeline => "agentpalace_kg_timeline",
            Self::KgStats => "agentpalace_kg_stats",
            Self::Traverse => "agentpalace_traverse",
            Self::FindTunnels => "agentpalace_find_tunnels",
            Self::GraphStats => "agentpalace_graph_stats",
            Self::Search => "agentpalace_search",
            Self::CheckDuplicate => "agentpalace_check_duplicate",
            Self::AddDrawer => "agentpalace_add_drawer",
            ...
            Self::TaskCreate => "agentpalace_task_create",
            Self::TaskList => "agentpalace_task_list",
            Self::TaskGet => "agentpalace_task_get",
            Self::TaskClaim => "agentpalace_task_claim",
            Self::TaskRenew => "agentpalace_task_renew",
            Self::TaskTransition => "agentpalace_task_transition",
            Self::MessageSend => "agentpalace_message_send",
            Self::MessageGet => "agentpalace_message_get",
            Self::MessageAcknowledge => "agentpalace_message_acknowledge",
            Self::InboxRead => "agentpalace_inbox_read",
            Self::ArtifactPut => "agentpalace_artifact_put",
            Self::ArtifactGet => "agentpalace_artifact_get",
            Self::ResultPut => "agentpalace_result_put",
            Self::ResultGet => "agentpalace_result_get",
            Self::CoordinationEventGet => "agentpalace_coordination_event_get",
            Self::CoordinationEvents => "agentpalace_coordination_events",
            Self::SkillPropose => "agentpalace_skill_propose",
            Self::SkillGet => "agentpalace_skill_get",
            Self::SkillVersions => "agentpalace_skill_versions",
            Self::SkillList => "agentpalace_skill_list",
            Self::SkillRecordOutcome => "agentpalace_skill_record_outcome",
            Self::SkillPromote => "agentpalace_skill_promote",
            Self::SkillRetire => "agentpalace_skill_retire",
            Self::SkillReviews => "agentpalace_skill_reviews",
            Self::DelegationSpanStart => "agentpalace_delegation_span_start",
            Self::DelegationSpanGet => "agentpalace_delegation_span_get",
            Self::DelegationSpanClose => "agentpalace_delegation_span_close",
            Self::DelegationSpansForTask => "agentpalace_delegation_spans_for_task",
            Self::DelegationCheckpointAppend => "agentpalace_delegation_checkpoint_append",
            Self::DelegationCheckpointGet => "agentpalace_delegation_checkpoint_get",
            Self::DelegationTrace => "agentpalace_delegation_trace",
            Self::LineageSet => "agentpalace_lineage_set",
            Self::SelfObservationPropose => "agentpalace_self_observation_propose",
            Self::SelfObservationReview => "agentpalace_self_observation_review",
            Self::IdentityPacket => "agentpalace_identity_packet",
            Self::MigrationRecord => "agentpalace_migration_record",
            Self::A2aAgentCard => "agentpalace_a2a_agent_card",
            Self::A2aTaskImport => "agentpalace_a2a_task_import",
            Self::A2aTaskExport => "agentpalace_a2a_task_export",
            Self::A2aMessageImport => "agentpalace_a2a_message_import",
            Self::A2aArtifactImport => "agentpalace_a2a_artifact_import",
            Self::McpTasksGet => "agentpalace_mcp_tasks_get",
            Self::McpTasksUpdate => "agentpalace_mcp_tasks_update",
            Self::McpTasksCancel => "agentpalace_mcp_tasks_cancel",
            Self::McpTasksImport => "agentpalace_mcp_tasks_import",
        }
    }
    ...
    fn definition(self) -> ToolDefinition {
        match self {
            ...
            Self::IdentityRead => ToolDefinition {
                name: self.as_str(),
                description: "Read the configured identity.txt used by agentpalace_wake_up.",
                input_schema: json!({"type":"object","properties":{}}),
            },
            ...
            Self::SkillList => coordination_definition(
                self,
                "Discover skills filtered by scope, status, and/or wing. Supplying `wing` hides project-scoped skills owned by other projects while keeping agent- and organization-scoped ones; omitting it spans every project and is an administrative view. Discovery only: dereference a specific version with agentpalace_skill_get before treating it as authoritative. limit is clamped to 1..=500.",
            ...
            Self::A2aTaskExport => coordination_definition(
                self,
                "Translate an authoritative AgentPalace task into an A2A Task for export, returning found:false for a miss. Artifacts and messages are not bulk-fetched — pass the exact artifact_ids/message_ids to include (fetched from agentpalace_coordination_events or already known to the caller); artifacts whose role is protocol_envelope are always excluded from the emitted A2A artifacts list, since they are audit records, not A2A artifacts. Returns {found, task, coercion} where coercion reports whether the task's AgentPalace state required coercion to reach its A2A counterpart (only Expired does, coerced to TASK_STATE_FAILED). Local-only: this is a read-and-translate operation with no A2A HTTP surface.",
            ...
            Self::McpTasksCancel => coordination_definition(
                self,
                "Cancel a task on behalf of an inbound MCP Tasks `tasks/cancel` request, under the same compare-and-swap revision semantics as agentpalace_task_transition. Equivalent to agentpalace_mcp_tasks_update with status fixed to cancelled. Cancelling an already-cancelled task succeeds as a no-op (reported as no_op:true) rather than failing, since no transition out of a terminal state is permitted; the expected_revision check still applies. Returns {\"success\": true, \"task\": {...}} on success, or {\"success\": false, \"conflict\": {expected_revision, actual_revision, message}} when the expected revision no longer matches — a conflict is data, not an error. Local-only: no MCP Tasks transport of its own.",
...
pub fn configured_lineage_id_from_env() -> Result<Option<String>> {
    match agentpalace_core::env_var(LINEAGE_ID_ENV) {
        Ok(value) => {
...
where
    P: EmbeddingProvider + Send + 'static,
{
    pub async fn from_parts(config: AgentPalaceConfig, provider: P) -> Result<Self> {
    ...
    P: EmbeddingProvider + Send,
{
    ...
    fn local_fact_state_from_storage(
        ...
        let rows = match runtime.query_entity(subject, None, QueryDirection::Outgoing) {
            Ok(rows) => rows,
            Err(agentpalace_graph::GraphError::UnknownEntity { .. }) => return Ok(None),
    ...
    fn compile_identity_packet(
                ...
                match lineage {
                    ...
                    None => {
                        ...
                        let message = if fallback_id.is_some() {
                            format!(
                                "{LINEAGE_ID_ENV} is set to `{lineage_id}`, but that lineage does not exist. The palace default is being used for this response. To create it, call agentpalace_lineage_set with lineage_id `{lineage_id}`, display_name, description, expected_revision 0, set_default false, and actor, then retry wake-up."
                            )
                        } else {
                            format!(
                                "{LINEAGE_ID_ENV} is set to `{lineage_id}`, but that lineage does not exist and no palace default is configured. Create it with agentpalace_lineage_set using lineage_id `{lineage_id}`, display_name, description, expected_revision 0, set_default false, and actor, then retry wake-up."
                            )
                        };
                        (
                            fallback,
        ...
        let Some(lineage) = lineage else {
            ...
            let message = match self.bound_lineage_id.as_deref() {
                ...
                None => "No default lineage is configured. Create one with agentpalace_lineage_set or bind the MCP server with AGENTPALACE_LINEAGE_ID.".to_owned(),
            };
            return Ok(json!({
                "packet_version": 1,
                "configured": false,
    ...
    async fn tool_delete_drawer(&mut self, arguments: &Value) -> ToolResult<Value> {
        ...
        // `change_event_visible` in `crates/agentpalace-server/src/lib.rs` and
    ...
    async fn tool_kg_query(&mut self, arguments: &Value) -> ToolResult<Value> {
        ...
        let facts = if local_read {
            match runtime.query_entity(&entity, as_of, direction) {
                ...
                Err(agentpalace_graph::GraphError::UnknownEntity { .. }) if federated_read => {
                    Vec::new()
                }
    ...
    /// lookup — like `agentpalace_get_changes_since`, it always reads local *and* (when
    ...
    /// its sibling `agentpalace_coordination_events` is `RoutableCoordination`.
    ...
    async fn tool_task_list(&mut self, arguments: &Value) -> ToolResult<Value> {
        ...
        let filter = agentpalace_storage::TaskListFilter {
            wing: wing.clone(),
            ...
            json!(
                self.coordination
                    .tasks(
                        ...
                        &filter,
                        limit,
                        agentpalace_storage::TASK_LIST_PAGE_BYTES,
                        CoordinationVisibility::Trusted
                    )
                    .map_tool()?
            )
        ...
        if let Some(router) = self.federation.as_ref().filter(|r| r.has_remotes()) {
            let cursors = parse_cursors_arg(arguments, "remote_cursors")?;
            let query = agentpalace_federation::CoordinationTasksQuery {
                cursor: None,
                wing,
                state,
                owner: filter.owner,
    ...
    /// `agentpalace_inbox_read` — always reads local and fans out to every configured remote
    ...
    /// fields `agentpalace-server`'s `/v1/info` route reports, sourced from local config rather
    /// than fabricated — `agentpalace-mcp` has no maintenance run history of its own to report, so
    ...
    /// `agentpalace_a2a::agent_card`'s module docs for why this crate cannot infer its own address.
    ...
    /// re-serialized value — see `agentpalace_a2a::envelope::envelope_artifact`'s doc comment for
    ...
    async fn tool_a2a_task_import(&mut self, arguments: &Value) -> ToolResult<Value> {
        ...
        let imported = self
            ...
            .map_tool_internal()?;
        reject_replay_state_mismatch(&imported, conversion.target_state.value)?;
    ...
    async fn tool_a2a_task_export(&mut self, arguments: &Value) -> ToolResult<Value> {
        ...
        for artifact_id in &artifact_ids {
            ...
            if artifact.role == agentpalace_a2a::PROTOCOL_ENVELOPE_ROLE {
                continue;
            }
        ...
        for message_id in &message_ids {
            ...
            history.push(agentpalace_a2a::message_to_a2a_message(&message).map_err(map_a2a_tool_error)?);
        }
    ...
    async fn tool_a2a_message_import(&mut self, arguments: &Value) -> ToolResult<Value> {
        ...
        if raw_message.len() > MAX_PAYLOAD_BYTES {
            return Err(ToolError::InvalidParams(format!(
                "`a2a_message` is {} bytes, over the {MAX_PAYLOAD_BYTES}-byte payload limit",
                raw_message.len()
            )));
        }
    ...
    async fn tool_a2a_artifact_import(&mut self, arguments: &Value) -> ToolResult<Value> {
        ...
        if raw_artifact.len() > MAX_PAYLOAD_BYTES {
            return Err(ToolError::InvalidParams(format!(
                "`a2a_artifact` is {} bytes, over the {MAX_PAYLOAD_BYTES}-byte payload limit",
                raw_artifact.len()
            )));
        }
    ...
    async fn tool_mcp_tasks_get(&mut self, arguments: &Value) -> ToolResult<Value> {
        ...
        let error: Option<agentpalace_mcp_tasks::JsonRpcErrorObject> = match arguments.get("error") {
            None | Some(Value::Null) => None,
            Some(value) => Some(
                serde_json::from_value(value.clone())
    ...
    async fn tool_mcp_tasks_update(&mut self, arguments: &Value) -> ToolResult<Value> {
        ...
        // `agentpalace_mcp_tasks_get` reports a `Pending` task as `working`, an MCP-only consumer
        ...
        // MCP Tasks updates are progress pings, and a client may legitimately resend the status
        // it last set. `allowed_transition` has no self-transition edge, so handing that to
        // `transition_task` would produce a `StorageError::Invariant` and surface as an internal
        // -32000 for what is a harmless no-op. Revision is still checked first, so a stale
        ...
        if current.state == target.value {
            ...
            if current.owner.is_some() && current.owner.as_deref() != Some(actor.as_str()) {
                return Err(ToolError::Internal(McpError::Storage(
                    agentpalace_storage::StorageError::Invariant(ONLY_OWNER_MAY_TRANSITION.into()),
                )));
            }
            return Ok(json!({
                "success": true,
                "task": current,
                "bridged_from_pending": false,
                "no_op": true,
            ...
            match self.coordination.claim_task(
                ...
                Ok(RevisionedWrite::Applied(task)) => {
                    ...
                    if target.value == TaskState::Running {
                        // `claim_task` records a `task_claimed` audit event, which carries no
                        // `details` field, so a `details` payload cannot be attached on this
                        // path. Report that rather than dropping it silently.
                        return Ok(json!({
                            "success": true,
                            "task": task,
                            "bridged_from_pending": true,
                            "no_op": false,
                            "details_recorded": details.is_none(),
                        }));
                    }
                }
                Ok(RevisionedWrite::Conflict { actual_revision }) => {
        ...
        match self.coordination.transition_task(
            ...
            Ok(RevisionedWrite::Applied(task)) => Ok(json!({
                "success": true,
                "task": task,
                "bridged_from_pending": bridged_from_pending,
                "no_op": false,
    ...
    /// lifecycle expiry (see `agentpalace_mcp_tasks::ttl`'s module docs).
...
/// Refuses an idempotent import replay whose stored task is in a different state than the one
/// the current payload maps to.
///
/// `find_task_by_key` matches on `(created_by, idempotency_key)` alone, so replaying a key with a
/// different inbound status would otherwise return the *first* payload's task while the response
...
fn reject_replay_state_mismatch(imported: &ImportedTask, requested: TaskState) -> ToolResult<()> {
    if imported.replayed && imported.task.state != requested {
        return Err(ToolError::InvalidParams(format!(
            ...
            imported.task.task_id, imported.task.state, requested
        )));
    }
    Ok(())
}
...
fn map_a2a_tool_error(error: A2aError) -> ToolError {
    match error {
        // A wrong `artifact_id`/`message_id` - a real record that simply was not created through
        // the A2A adapter, e.g. an ordinary `agentpalace_artifact_put` with role "output" - lands
        // here. That is a caller mistake, not storage corruption, so it must not be a -32000
        // internal error.
        A2aError::InvalidStoredShape { .. } => ToolError::InvalidParams(error.to_string()),
...
/// Render a [`agentpalace_a2a::Coercion`] (or its absence) as JSON. `Coercion` itself is not
/// `Serialize` — its whole point is that callers must engage with `Option<Coercion>` explicitly
/// (see `agentpalace_a2a::state::Mapped`'s doc comment) rather than something that silently
/// serializes to `null`.
fn a2a_coercion_json(coercion: Option<agentpalace_a2a::Coercion>) -> Value {
    match coercion {
        None => Value::Null,
...
/// Render a [`agentpalace_mcp_tasks::Coercion`] (or its absence) as JSON. See
/// [`a2a_coercion_json`]'s doc comment — same reasoning, independent type.
fn mcp_tasks_coercion_json(coercion: Option<agentpalace_mcp_tasks::Coercion>) -> Value {
    match coercion {
        None => Value::Null,
        Some(c) => json!({"from": c.from, "to": c.to, "reason": c.reason}),
    }
...
/// Converts a local `agentpalace_storage::TaskState` to the wire `CoordinationTaskState` used by
/// `TransitionTaskRequest`, mirroring `wire_task_state` in `crates/agentpalace-server/src/lib.rs`
...
/// `agentpalace_storage::coordination`) — the signal a federated coordination tool uses to decide
...
/// propagate as-is.
///
/// Matches against [`agentpalace_storage::NOT_FOUND_SUFFIX`] rather than a bare `"not found"`
...
fn is_local_record_missing(err: &agentpalace_storage::StorageError) -> bool {
    matches!(
        err,
        agentpalace_storage::StorageError::Invariant(msg)
            if msg.contains(agentpalace_storage::NOT_FOUND_SUFFIX)
    )
}

/// Parses an optional `{remote_name: cursor}` object argument into a per-remote cursor map, the
/// same shape `agentpalace_get_changes_since`'s inline `cursors` parsing already uses — factored
...
/// entities and predicates (agentpalace-graph `canonicalize_label`): lowercase
...
fn hash_text(content: &str) -> String {
    agentpalace_core::hash_text(content)
}
...
pub fn phase0_tools_fixture() -> Result<Value> {
    ...
    serde_json::from_str(&body.replace("mempalace", "agentpalace").replace("MemPalace", "AgentPalace")).map_err(McpError::from)
}
...
mod tests {
    ...
    use agentpalace_embeddings::{StartupValidation, StartupValidationStatus};
    ...
    impl EmbeddingProvider for BlockingProvider {
        ...
        fn embed(
            ...
            agentpalace_embeddings::EmbeddingResponse::from_vectors(
    ...
    async fn federated_branch_overrides_suppress_only_matching_wing_and_path() {
        ...
        for (id, source_file, path_state) in [
            ...
            override_row.view_metadata = Some(agentpalace_core::RepositoryViewMetadata {
    ...
    async fn tool_inventory_matches_phase0_fixture_with_current_branding() {
        ...
        for (name, definition) in expected {
            // The fixture loader maps the frozen upstream product namespace.
            // Everything else, including every input schema, compares exactly.
    ...
    fn identity_tools_do_not_expose_model_selectable_lineage_ids() {
        for tool_name in ["agentpalace_wake_up", "agentpalace_identity_packet"] {
    ...
    /// `agentpalace_inbox_read` and `agentpalace_coordination_events` must declare a `remote_cursors`
    ...
    fn inbox_read_and_coordination_events_declare_remote_cursors_schema() {
        for tool_name in ["agentpalace_inbox_read", "agentpalace_coordination_events"] {
    ...
    async fn native_coordination_tools_support_exact_recovery_and_inbox_acknowledgement() {
        ...
        let inbox = harness
            .server
            .handle_request(tool_call(903, "agentpalace_inbox_read", json!({"recipient":"worker"})))
            .await;
        assert_eq!(
        ...
        let exact = harness
            .server
            .handle_request(tool_call(905, "agentpalace_task_get", json!({"task_id":task_id})))
            .await;
        assert_eq!(decode_tool_payload(&exact).expect("exact")["found"], true);
        let missing = harness
            .server
            .handle_request(tool_call(906, "agentpalace_task_get", json!({"task_id":"task_missing"})))
            .await;
        ...
        let exact_result_response = harness
            .server
            .handle_request(tool_call(908, "agentpalace_result_get", json!({"result_id":result_id})))
            .await;
        assert_eq!(
    ...
    async fn task_create_rejects_non_object_arguments_without_panicking() {
        ...
        let response = harness
            .server
            .handle_request(tool_call(910, "agentpalace_task_create", json!([])))
            .await;
    ...
    async fn coordination_wings_empty_palace_reports_builtin_local_default() {
        ...
        .unwrap();
        let response = server
            .handle_request(tool_call(913, "agentpalace_coordination_wings", json!({})))
            .await;
    ...
    async fn coordination_wings_marks_configured_default_without_route() {
        ...
        let response = harness
            .server
            .handle_request(tool_call(914, "agentpalace_coordination_wings", json!({})))
            .await;
        let payload = decode_tool_payload(&response).expect("discovery payload");
    ...
    async fn coordination_wings_discovers_task_only_wing_as_in_use() {
        ...
        let response = harness
            .server
            .handle_request(tool_call(916, "agentpalace_coordination_wings", json!({})))
            .await;
        let payload = decode_tool_payload(&response).expect("discovery payload");
        let entry = payload["wings"]
            .as_array()
    ...
    async fn task_create_wing_agents_short_form_stays_local_and_never_calls_remote() {
        for raw_wing in ["agents", "Wing_Agents", " wing_agents "] {
            ...
            let mut remotes: BTreeMap<String, Arc<dyn agentpalace_remote::RemoteApi>> =
                BTreeMap::new();
    ...
    /// fan-out gate regression tests below (`agentpalace_inbox_read`/`agentpalace_coordination_events`
    ...
    /// Codex P1 finding (comment 3832912220), Part 1: `agentpalace_inbox_read`'s aggregate
    ...
    async fn inbox_read_stays_local_without_coordination_federation_configured() {
        ...
        let response = harness
            .server
            .handle_request(tool_call(930, "agentpalace_inbox_read", json!({"recipient": "worker"})))
            .await;
        ...
        assert_eq!(
            observed, 0,
            "agentpalace_inbox_read must not contact a remote configured for drawers only, with \
             an empty federation.coordination table and default_mode: local, got {observed} \
             remote call(s)"
        );
    }

    /// Codex P1 finding (comment 3832912220), Part 2: `agentpalace_coordination_events`
    ...
    async fn coordination_events_stays_local_without_coordination_federation_configured() {
        ...
        let response = harness
            .server
            .handle_request(tool_call(931, "agentpalace_coordination_events", json!({})))
            .await;
        ...
        assert_eq!(
            observed, 0,
            "agentpalace_coordination_events must not contact a remote configured for drawers \
    ...
    /// enabled (`default_mode: remote`), `agentpalace_inbox_read` must never forward a
    ...
    /// Codex P1 finding (comment 3832912230), `agentpalace_coordination_events` counterpart —
    ...
    /// and an ordinary (non-diary) wing, `agentpalace_inbox_read` DOES fan out to the configured
    ...
    /// PR #120 review, finding 1(a): `agentpalace_inbox_read` and `agentpalace_coordination_events`
    ...
    async fn inbox_read_and_coordination_events_fanout_only_contact_the_coordination_candidate() {
        ...
        let inbox = harness
            .server
            .handle_request(tool_call(940, "agentpalace_inbox_read", json!({"recipient": "worker"})))
            .await;
        ...
        assert_eq!(
            ...
            "other is never named by any federation.coordination rule and must not be contacted \
             by agentpalace_inbox_read"
        );

        let events = harness
            .server
            .handle_request(tool_call(941, "agentpalace_coordination_events", json!({})))
            .await;
        ...
        assert_eq!(
            hub_calls.load(std::sync::atomic::Ordering::SeqCst),
            2,
            "hub must be contacted again for agentpalace_coordination_events"
        );
        assert_eq!(
            other_calls.load(std::sync::atomic::Ordering::SeqCst),
            0,
    ...
    async fn coordination_fanout_distinguishes_capability_missing_from_unreachable() {
        ...
        let response = harness
            .server
            .handle_request(tool_call(942, "agentpalace_inbox_read", json!({"recipient": "worker"})))
            .await;
        ...
        let response = harness
            .server
            .handle_request(tool_call(943, "agentpalace_inbox_read", json!({"recipient": "worker"})))
            .await;
        let payload = decode_tool_payload(&response).expect("inbox payload");
        let remote_result = payload["remote_messages"]["hub"].clone();
        assert_eq!(
    ...
    /// Reproduces the live regression end to end, through the tool layer (`agentpalace_task_get`),
    ...
    ///
    /// Before this fix, `agentpalace_task_get` on an unknown task id hard-errored with
    ...
    /// (`agentpalace_message_get`, `agentpalace_artifact_get`, `agentpalace_result_get`) went through
    /// the exact same `coordination_read_fallback` helper and was equally broken; this test only
    /// drives `agentpalace_task_get` because all four share one fallback implementation.
    #[tokio::test]
    ...
    /// error `agentpalace_storage::coordination::CoordinationStore` actually produces for a
    ...
    /// `agentpalace_storage::NOT_FOUND_SUFFIX`, the pinned constant both this predicate and
    /// `agentpalace-server`'s `coordination_storage_error` match against. Before the fix, the
    ...
    fn is_local_record_missing_matches_real_missing_task_and_message_errors() {
        let tempdir = TempDir::new().unwrap();
        let store =
            agentpalace_storage::CoordinationStore::new(tempdir.path().join("storage.sqlite3"));
        ...
        match &err {
            agentpalace_storage::StorageError::Invariant(msg) => {
                assert!(
                    msg.ends_with(agentpalace_storage::NOT_FOUND_SUFFIX),
    ...
    async fn task_list_mcp_discovery_to_claim_and_independent_local_cursor() {
        let harness = test_harness().await;
        for i in 0..3 {
            let response = harness.server.handle_request(tool_call(910+i,"agentpalace_task_create",
        ...
        let claimed = harness.server.handle_request(tool_call(923,"agentpalace_task_claim",json!({"task_id":row["task_id"],"expected_revision":row["revision"],"worker":"worker","lease_seconds":60}))).await;
        ...
        let propose = |id: i64, key: &str| {
            tool_call(
                ...
                json!({
                    ...
                    "instructions_ref":"skills/coordinate-with-agentpalace/SKILL.md",
    ...
    /// docs/Coordination-Phase-3-Design.md: before the fix, `agentpalace_skill_propose` stored a
    /// raw wing while `agentpalace_skill_list` normalised its filter, so proposing and listing
    ...
    async fn skill_propose_and_list_agree_on_an_unprefixed_wing() {
        ...
        let list = harness
            .server
            .handle_request(tool_call(931, "agentpalace_skill_list", json!({"wing":"myproject"})))
            .await;
    ...
    async fn search_tool_resolves_locator_rows_and_marks_stale_only_when_changed() {
        ...
        let make_row = |id: &str, room: &str, file_hash: String| DrawerRecord {
            ...
            embedding: vec![1.0; EmbeddingProfile::Balanced.metadata().dimensions],
            locator: Some(agentpalace_core::SourceLocator {
                byte_start: 0,
        ...
        let fresh =
            make_row("wing_mined/fresh/0001", "fresh", agentpalace_core::hash_bytes(file_body));
    ...
    async fn status_tool_reports_protocol_and_counts() {
        let harness = test_harness().await;
        let response =
            harness.server.handle_request(tool_call(3, "agentpalace_status", json!({}))).await;
        let payload = decode_tool_payload(&response).unwrap();
    ...
    async fn mcp_bound_lineage_cannot_be_overridden_by_tool_arguments() {
        ...
        let packet = decode_tool_payload(
            &harness
                .server
                .handle_request(tool_call(6072, "agentpalace_identity_packet", json!({})))
                .await,
        )
        .unwrap();
    ...
    async fn missing_mcp_bound_lineage_falls_back_to_default_with_creation_guidance() {
        ...
        let response = harness
            .server
            .handle_request(tool_call(6075, "agentpalace_identity_packet", json!({})))
            .await;
        ...
        assert!(message.contains("agentpalace_lineage_set"));
        assert!(message.contains("expected_revision 0"));

        let wake = decode_tool_payload(
            &harness.server.handle_request(tool_call(6076, "agentpalace_wake_up", json!({}))).await,
        )
        .unwrap();
        assert_eq!(
        ...
        let rebound = decode_tool_payload(
            &harness
                .server
                .handle_request(tool_call(6078, "agentpalace_identity_packet", json!({})))
                .await,
        )
        .unwrap();
    ...
    async fn lineage_observations_migrations_and_wake_up_form_a_portable_identity_packet() {
        ...
        let changes = decode_tool_payload(
            &harness
                .server
                .handle_request(tool_call(617, "agentpalace_get_changes_since", json!({"limit":100})))
                .await,
        )
        .unwrap();
    ...
    async fn identity_update_supports_replace_and_append() {
        ...
        let read = harness
            .server
            .handle_request(tool_call(612, "agentpalace_identity_read", json!({})))
            .await;
        ...
        let changes = harness
            .server
            .handle_request(tool_call(613, "agentpalace_get_changes_since", json!({"limit": 10})))
            .await;
    ...
    async fn diary_summary_is_bounded_and_wake_up_supports_detail_lookup() {
        ...
        let wake = decode_tool_payload(
            &harness
                .server
                .handle_request(tool_call(82, "agentpalace_wake_up", json!({"diary_limit":1})))
                .await,
        )
        .unwrap();
        ...
        let detail = decode_tool_payload(
            &harness
                .server
                .handle_request(tool_call(83, "agentpalace_diary_read", json!({"entry_id":entry_id})))
                .await,
        )
        .unwrap();
    ...
    async fn wake_up_uses_first_400_characters_for_legacy_diary_entries() {
        ...
        let wake = decode_tool_payload(
            &harness
                .server
                .handle_request(tool_call(85, "agentpalace_wake_up", json!({"diary_limit":1})))
                .await,
        )
        .unwrap();
    ...
    async fn diary_write_stores_project_and_agent_entries_in_context_wings() {
        ...
        let first = harness
            ...
            .handle_request(tool_call(
                90,
                "agentpalace_diary_write",
                json!({"agent_name":"Worker-One","entry":"SESSION:project","summary":"Project session.","topic":"ops","scope":"project","wing":"wing_agentpalace-rs"}),
            ))
            .await;
        let second = harness
            .server
        ...
        let project_entries = decode_tool_payload(
            &harness
                ...
                .handle_request(tool_call(
                    92,
                    "agentpalace_diary_read",
                    json!({"wing":"wing_agentpalace-rs","last_n":10}),
                ))
                .await,
        )
        .unwrap();
        ...
        assert_eq!(project_entries["entries"][0]["wing"], "wing_agentpalace-rs");
    ...
    async fn taxonomy_listing_and_graph_tools_cover_seeded_data() {
        ...
        let list_wings =
            harness.server.handle_request(tool_call(12, "agentpalace_list_wings", json!({}))).await;
        let list_rooms = harness
            .server
            .handle_request(tool_call(13, "agentpalace_list_rooms", json!({"wing":"wing_code"})))
            .await;
        let list_all_rooms =
            harness.server.handle_request(tool_call(130, "agentpalace_list_rooms", json!({}))).await;
        let taxonomy =
            harness.server.handle_request(tool_call(14, "agentpalace_get_taxonomy", json!({}))).await;
        let aaak = harness
            .server
            .handle_request(tool_call(131, "agentpalace_get_aaak_spec", json!({})))
            .await;
        ...
        let tunnels = harness
            ...
            .await;
        let graph_stats =
            harness.server.handle_request(tool_call(17, "agentpalace_graph_stats", json!({}))).await;
    ...
    async fn knowledge_graph_tools_cover_add_query_invalidate_timeline_and_stats() {
        ...
        assert_eq!(query_payload["facts"][0]["object"], "AgentPalace");
        assert_eq!(query_payload["facts"][0]["source_closet"], source_closet);

        let timeline = harness
            .server
            .handle_request(tool_call(20, "agentpalace_kg_timeline", json!({"entity":"Alice Smith"})))
            .await;
        ...
        let full_timeline =
            harness.server.handle_request(tool_call(200, "agentpalace_kg_timeline", json!({}))).await;
        ...
        let stats =
            harness.server.handle_request(tool_call(22, "agentpalace_kg_stats", json!({}))).await;
    ...
    async fn serve_transport_processes_tool_calls_and_ignores_notifications() {
        ...
        let input = concat!(
            ...
            "{\"jsonrpc\":\"2.0\",\"id\":2,\"method\":\"tools/call\",\"params\":{\"name\":\"agentpalace_status\",\"arguments\":{}}}\n"
        );
    ...
    async fn stdio_maintenance_respects_writes_and_competing_lease() {
        use agentpalace_storage::MaintenanceLeaseStore;
        let harness = test_harness().await;
    ...
    fn entity_kind_heuristic_prefers_unknown_over_false_people() {
        ...
        assert_eq!(infer_entity_kind("AgentPalace"), EntityKind::Unknown);
    ...
    async fn wing_param_normalizes_symmetrically_across_writes_and_reads() {
        ...
        for wing_input in ["ghosttest", "wing_ghosttest", "GhostTest"] {
            let rooms = harness
                .server
                .handle_request(tool_call(401, "agentpalace_list_rooms", json!({"wing": wing_input})))
                .await;
        ...
        let wings =
            harness.server.handle_request(tool_call(403, "agentpalace_list_wings", json!({}))).await;
    ...
    async fn a2a_agent_card_lists_one_skill_per_wing() {
        let harness = test_harness().await;
        let response =
            harness.server.handle_request(tool_call(9000, "agentpalace_a2a_agent_card", json!({}))).await;
    ...
    /// `Expired` (see `agentpalace_a2a::state::map_inbound_task_state`), so this exercises the
    ...
    async fn a2a_task_import_cannot_reach_the_storage_layer_with_expired_as_a_target() {
        ...
        assert!(matches!(err, agentpalace_storage::StorageError::Invariant(_)));
    }
    ...
    async fn a2a_agent_card_includes_coordination_only_wings() {
        ...
        let response =
            harness.server.handle_request(tool_call(9071, "agentpalace_a2a_agent_card", json!({}))).await;
        let card = decode_tool_payload(&response).expect("agent card payload");
        let skills = card["skills"].as_array().expect("skills array");
        let ids: Vec<&str> = skills.iter().filter_map(|s| s["id"].as_str()).collect();
    ...
    async fn a2a_task_import_rejects_payload_exceeding_1_mib_before_creating_task() {
        ...
        let tasks_check = harness
            .server
            .handle_request(tool_call(9079, "agentpalace_task_get", json!({"task_id": "large"})))
            .await;
    ...
    async fn mcp_tasks_get_reports_outbound_coercions_for_pending_and_expired() {
        ...
        let pending_get = harness
            .server
            .handle_request(tool_call(9101, "agentpalace_mcp_tasks_get", json!({"task_id": task_id})))
            .await;
    ...
    /// `agentpalace_mcp_tasks_get` reports a `Pending` task as MCP status `working`, but
    ...
    /// MCP Tasks updates are progress pings and a client may resend the status it last set.
    /// `allowed_transition` has no self-transition edge, so this must be a no-op rather than an
    /// internal error.
    #[tokio::test]
    async fn mcp_tasks_update_with_the_current_status_is_an_idempotent_no_op() {
        let harness = test_harness().await;
        let created = harness
            .server
            .handle_request(tool_call(
                ...
                json!({
                    "title": "t", "description": "d", "created_by": "alice",
                    "wing": "wing_myproject", "idempotency_key": "mcp-tasks-noop-1",
                }),
            ))
            .await;
        let task_id =
        ...
        let first = harness
            ...
            .await;
        let first = decode_tool_payload(&first).expect("first update");
        ...
        let second = harness
            ...
            .await;
        let second = decode_tool_payload(&second).expect("second update");
        assert_eq!(second["success"], true, "{second}");
        assert_eq!(second["no_op"], true);
        assert_eq!(second["task"]["state"], "running");
    ...
    /// different inbound status would otherwise return the first payload's task while reporting
    /// the second payload's state, and file a second envelope disagreeing with the first.
    #[tokio::test]
    async fn a2a_task_import_rejects_a_replay_whose_state_disagrees_with_the_stored_task() {
        let harness = test_harness().await;
        let args = |raw: &str| {
            json!({
                "a2a_task": raw,
                "wing": "wing_myproject",
                "created_by": "alice",
                "idempotency_key": "a2a-replay-state-1",
                "title": "t",
                "description": "d",
            })
        };
        ...
        let first = harness
            ...
            .handle_request(tool_call(
                9180,
                "agentpalace_a2a_task_import",
                args(r#"{"id":"src1","status":{"state":"TASK_STATE_COMPLETED"}}"#),
            ))
            .await;
        let first = decode_tool_payload(&first).expect("first import");
        assert_eq!(first["replayed"], false);
        assert_eq!(first["task"]["state"], "completed");
        ...
        let replay = harness
            ...
            .await;
        let replay = decode_tool_payload(&replay).expect("replay");
        ...
        let mismatch = harness
            ...
            .handle_request(tool_call(
                9182,
                "agentpalace_a2a_task_import",
                args(r#"{"id":"src1","status":{"state":"TASK_STATE_FAILED"}}"#),
            ))
            .await;
        assert_eq!(mismatch["error"]["code"], ErrorCode::InvalidParams as i32, "{mismatch}");
    }

    /// A real record that simply was not created through the A2A adapter is a caller mistake
    /// (wrong id), not storage corruption, so it must be -32602 rather than -32000.
    #[tokio::test]
    async fn a2a_task_export_rejects_a_non_a2a_artifact_id_as_invalid_params() {
        let harness = test_harness().await;
        let created = harness
            .server
            .handle_request(tool_call(
                ...
                json!({
                    "title": "t", "description": "d", "created_by": "alice",
                    "wing": "wing_myproject", "idempotency_key": "a2a-wrong-id-1",
                }),
            ))
            .await;
        let task_id =
        ...
        let plain = harness
            ...
            .handle_request(tool_call(
                ...
                "agentpalace_artifact_put",
                json!({
                    "task_id": task_id, "created_by": "alice", "role": "output",
                    "media_type": "text/plain",
                    "content": "not an a2a artifact", "idempotency_key": "plain-artifact-1",
                }),
            ))
            .await;
        let artifact_id = decode_tool_payload(&plain).expect("artifact")["artifact_id"]
            .as_str()
            .unwrap()
            .to_owned();
        ...
        let exported = harness
            ...
            .handle_request(tool_call(
                9187,
                "agentpalace_a2a_task_export",
                json!({"task_id": task_id, "artifact_ids": [artifact_id]}),
            ))
            .await;
        assert_eq!(exported["error"]["code"], ErrorCode::InvalidParams as i32, "{exported}");
    }
    ...
    async fn mcp_tasks_update_bridges_a_pending_task_into_running() {
        ...
        let shown = harness
            .server
            .handle_request(tool_call(9151, "agentpalace_mcp_tasks_get", json!({"task_id": task_id})))
            .await;
    ...
    /// `agentpalace-server` advertises on `/v1/info` - `ingest` included.
    #[tokio::test]
    async fn a2a_agent_card_advertises_the_same_capabilities_as_the_server() {
        let harness = test_harness().await;
        let card = harness
            .server
            .handle_request(tool_call(9165, "agentpalace_a2a_agent_card", json!({})))
            .await;
    ...
    /// produces `Expired` (see `agentpalace_mcp_tasks::status::map_inbound_task_status`), so this
    ...
    async fn mcp_tasks_import_rejects_payload_exceeding_1_mib_before_creating_task() {
        ...
        let tasks_check = harness
            .server
            .handle_request(tool_call(9151, "agentpalace_task_get", json!({"task_id": "src-large"})))
            .await;
        assert_eq!(decode_tool_payload(&tasks_check).unwrap()["found"], false);
    }
    ...
    async fn federation_routing_does_not_panic_with_wing_rule() {
        ...
        let response = harness
            .server
            .handle_request(tool_call(1001, "agentpalace_list_rooms", json!({"wing": "wing_code"})))
            .await;
        let payload = decode_tool_payload(&response).unwrap();
    ...
    async fn coordination_availability_emitted_consistently_across_wing_tools() {
        ...
        let checks: [(&str, Value); 4] = [
            ("agentpalace_status", json!({})),
            ("agentpalace_list_wings", json!({})),
            ("agentpalace_list_rooms", json!({"wing": "wing_code"})),
            ("agentpalace_get_taxonomy", json!({})),
        ];
    ...
    async fn federation_none_produces_byte_identical_responses() {
        ...
        let tools = [
            ...
            ("agentpalace_list_rooms", json!({"wing": "wing_code"})),
            ("agentpalace_get_taxonomy", json!({})),
            ("agentpalace_search", json!({"query": "auth migration parity", "limit": 2})),
            ("agentpalace_kg_stats", json!({})),
        ];
        ...
        // agentpalace_check_duplicate — use content that matches a seeded drawer.
        let seeded_content = "Code notes: auth-migration keeps search filter semantics exact while storage changes underneath.";
        {
            ...
            assert_eq!(
                ...
                "agentpalace_check_duplicate produced different responses"
            );
        }

        // agentpalace_kg_query — use the seeded entity "Rust Rewrite".
        {
            let default_resp = harness_default
                .server
                .handle_request(tool_call(
            ...
            assert_eq!(
                decode_tool_payload(&default_resp).unwrap(),
                decode_tool_payload(&none_fed_resp).unwrap(),
                "agentpalace_kg_query produced different responses"
            );
        }

        // agentpalace_kg_timeline — full timeline (no entity filter).
        {
            let default_resp = harness_default
                .server
                .handle_request(tool_call(2014, "agentpalace_kg_timeline", json!({})))
                .await;
            let none_fed_resp = harness_none_fed
                .server
                .handle_request(tool_call(2015, "agentpalace_kg_timeline", json!({})))
                .await;
            assert_eq!(
                decode_tool_payload(&default_resp).unwrap(),
                decode_tool_payload(&none_fed_resp).unwrap(),
                "agentpalace_kg_timeline produced different responses"
            );
        }

        // ── Key-set comparison for one mutating call per harness ──────────────
        // agentpalace_add_drawer — fresh unique content so it succeeds.
        ...
        assert_eq!(
            ...
            "agentpalace_add_drawer top-level keys differ between federation:none and no federation"
        );
    }
    ...
    struct LibMockRemote {
        changes_events: Vec<agentpalace_federation::ChangeEventDto>,
        changes_next_cursor: Option<String>,
        search_results: Vec<agentpalace_federation::RemoteDrawerResult>,
        fail: bool,
    ...
    impl LibMockRemote {
        fn mutation_result<T>(&self) -> agentpalace_remote::Result<T> {
            if self.mutation_unknown_outcome {
                Err(agentpalace_remote::RemoteError::UnknownOutcome {
                    remote: "mock".to_owned(),
                    message: "committed but response lost".to_owned(),
                })
            } else {
                Err(agentpalace_remote::RemoteError::Unreachable {
                    remote: "mock".to_owned(),
    ...
    impl agentpalace_remote::RemoteApi for LibMockRemote {
        async fn info(&self) -> agentpalace_remote::Result<agentpalace_federation::InfoResponse> {
            Err(agentpalace_remote::RemoteError::Unreachable {
                remote: "mock".to_owned(),
                message: "not used".to_owned(),
            })
        ...
        async fn search_drawers(
            &self,
            _req: agentpalace_federation::DrawerSearchRequest,
        ) -> agentpalace_remote::Result<agentpalace_federation::DrawerSearchResponse> {
            Ok(agentpalace_federation::DrawerSearchResponse { results: self.search_results.clone() })
        }
        async fn check_duplicate(
            &self,
            _req: agentpalace_federation::CheckDuplicateRequest,
        ) -> agentpalace_remote::Result<agentpalace_federation::CheckDuplicateResponse> {
            Ok(agentpalace_federation::CheckDuplicateResponse {
                is_duplicate: false,
                matches: json!([]),
            })
        }
        async fn add_drawer(
            &self,
            _req: agentpalace_federation::AddDrawerRequest,
        ) -> agentpalace_remote::Result<agentpalace_federation::AddDrawerResponse> {
            self.mutation_result()
        }
        async fn list_drawers(
            &self,
            _query: agentpalace_federation::ListDrawersQuery,
        ) -> agentpalace_remote::Result<agentpalace_federation::ListDrawersResponse> {
            Ok(agentpalace_federation::ListDrawersResponse { drawers: json!([]), next_cursor: None })
        }
        async fn get_drawer(&self, _drawer_id: &str) -> agentpalace_remote::Result<Value> {
            Ok(json!({}))
        }
        ...
        async fn kg_query(
            &self,
            _req: agentpalace_federation::KgQueryRequest,
        ) -> agentpalace_remote::Result<Value> {
            Ok(json!({"entity":"","facts":[],"count":0}))
        }
        async fn kg_add_fact(
            &self,
            _req: agentpalace_federation::KgAddFactRequest,
        ) -> agentpalace_remote::Result<Value> {
            self.mutation_result()
        }
        async fn kg_invalidate(
            &self,
            _req: agentpalace_federation::KgInvalidateRequest,
        ) -> agentpalace_remote::Result<Value> {
            self.mutation_result()
        }
        async fn kg_timeline(&self, _entity: Option<&str>) -> agentpalace_remote::Result<Value> {
            Ok(json!({"entity":"all","timeline":[],"count":0}))
        }
        async fn kg_stats(&self) -> agentpalace_remote::Result<Value> {
            Ok(
                json!({"entities":0,"triples":0,"current_facts":0,"expired_facts":0,"relationship_types":[]}),
            )
        }
        async fn taxonomy(&self) -> agentpalace_remote::Result<Value> {
            Ok(json!({"taxonomy":{}}))
        }
        async fn wings(&self) -> agentpalace_remote::Result<Value> {
            Ok(json!({"wings":{}}))
        }
        async fn rooms(&self, _wing: Option<&str>) -> agentpalace_remote::Result<Value> {
            Ok(json!({"rooms":{}}))
        }
        async fn changes(
            &self,
            _query: agentpalace_federation::ChangesQuery,
        ) -> agentpalace_remote::Result<agentpalace_federation::ChangesResponse> {
            if self.fail {
                return Err(agentpalace_remote::RemoteError::Unreachable {
                    remote: "mock".to_owned(),
                    message: "mock remote is down".to_owned(),
                });
            }
            Ok(agentpalace_federation::ChangesResponse {
        ...
        async fn ingest_batch(
            &self,
            _req: agentpalace_federation::IngestBatchRequest,
        ) -> agentpalace_remote::Result<agentpalace_federation::IngestBatchResponse> {
            Err(agentpalace_remote::RemoteError::Unreachable {
                remote: "mock".to_owned(),
                message: "not used".to_owned(),
            })
        ...
        async fn coordination_task_create(
            &self,
            req: agentpalace_federation::NewTaskRequest,
        ) -> agentpalace_remote::Result<agentpalace_federation::CoordinationTaskDto> {
        ...
        async fn coordination_inbox(
            &self,
            query: agentpalace_federation::InboxQuery,
        ) -> agentpalace_remote::Result<agentpalace_federation::InboxPageResponse> {
            self.coordination_calls.fetch_add(1, std::sync::atomic::Ordering::SeqCst);
            let _ = query;
            match self.coordination_fanout_outcome {
                LibMockFanoutOutcome::Success => Ok(agentpalace_federation::InboxPageResponse {
                    messages: vec![],
                    next_cursor: None,
                }),
                LibMockFanoutOutcome::CapabilityMissing => {
                    Err(agentpalace_remote::RemoteError::CapabilityMissing {
                        remote: "mock".to_owned(),
        ...
        async fn coordination_events(
            &self,
            query: agentpalace_federation::CoordinationEventsQuery,
        ) -> agentpalace_remote::Result<agentpalace_federation::CoordinationEventsResponse> {
            self.coordination_calls.fetch_add(1, std::sync::atomic::Ordering::SeqCst);
            let _ = query;
            match self.coordination_fanout_outcome {
                LibMockFanoutOutcome::Success => {
                    Ok(agentpalace_federation::CoordinationEventsResponse {
                        events: vec![],
                        next_cursor: None,
                    })
                }
    ...
    fn make_lib_router(
        remotes: BTreeMap<String, Arc<dyn agentpalace_remote::RemoteApi>>,
    ) -> FederationRouter {
        let mut rules_remotes = BTreeMap::new();
        for name in remotes.keys() {
            rules_remotes.insert(
    ...
    async fn remote_only_view_search_keeps_remote_results_despite_local_branch_rows() {
        ...
        remote.search_results = vec![agentpalace_federation::RemoteDrawerResult {
        ...
        local_branch_row.view_metadata = Some(agentpalace_core::RepositoryViewMetadata {
            repo_id: "repo".to_owned(),
            view_name: Some("feature-x".to_owned()),
            source_path: "/repo".to_owned(),
            head_commit: Some("head".to_owned()),
    ...
    fn make_dto_event(
        ...
        occurred_at: &str,
        entity_id: &str,
    ) -> agentpalace_federation::ChangeEventDto {
        agentpalace_federation::ChangeEventDto {
    ...
    async fn tool_wake_up_with_federation_includes_remote_changes() {
        ...
        let payload = decode_tool_payload(
            &harness.server.handle_request(tool_call(9030, "agentpalace_wake_up", json!({}))).await,
        )
        .unwrap();
    ...
    async fn tool_wake_up_with_unreachable_remote_still_succeeds() {
        ...
        let payload = decode_tool_payload(
            &harness.server.handle_request(tool_call(9040, "agentpalace_wake_up", json!({}))).await,
        )
        .unwrap();
    ...
    async fn federation_none_wake_up_has_no_remote_changes_key() {
        let harness = test_harness().await;
        let payload = decode_tool_payload(
            &harness.server.handle_request(tool_call(9100, "agentpalace_wake_up", json!({}))).await,
        )
        .unwrap();
        assert!(
    ...
    //
    // These tests verify that `agentpalace_delete_drawer`:
    ...
    #[async_trait::async_trait]
    impl agentpalace_remote::RemoteApi for DeleteDrawerMock {
        async fn info(&self) -> agentpalace_remote::Result<agentpalace_federation::InfoResponse> {
        ...
        async fn delete_drawer(&self, _drawer_id: &str) -> agentpalace_remote::Result<()> {
            ...
            if self.delete_succeeds {
                ...
                Err(agentpalace_remote::RemoteError::RemoteRejected {
                    remote: "mock".to_owned(),
                    status: 404,
        ...
        async fn kg_stats(&self) -> agentpalace_remote::Result<serde_json::Value> {
            panic!("unexpected kg_stats call")
        }
        async fn taxonomy(&self) -> agentpalace_remote::Result<serde_json::Value> {
            panic!("unexpected taxonomy call")
        }
        async fn wings(&self) -> agentpalace_remote::Result<serde_json::Value> {
            panic!("unexpected wings call")
        }
        async fn rooms(&self, _wing: Option<&str>) -> agentpalace_remote::Result<serde_json::Value> {
    ...
    async fn tool_delete_drawer_with_write_remote_local_hit() {
        ...
        let remotes =
            BTreeMap::from([("alpha".to_owned(), mock as Arc<dyn agentpalace_remote::RemoteApi>)]);
        ...
        ctx.runtime
            ...
            .put_drawers(
                &[DrawerRecord {
                    ...
                    content_hash: agentpalace_core::hash_text("test content"),
                    embedding: vec![0.0; EmbeddingProfile::Balanced.metadata().dimensions],
                    locator: None,
                    view_metadata: None,
                }],
        ...
        ctx.runtime
            ...
            .put_drawers(
                &[DrawerRecord {
                    ...
                    weight: None,
                    content: content.to_owned(),
                    content_hash: agentpalace_core::hash_text(content),
                    embedding: vec![0.0; EmbeddingProfile::Balanced.metadata().dimensions],
                    locator: None,
                    view_metadata: None,
                }],
    ...
    async fn keyed_kg_operations_reuse_already_activated_outbox_rows() {
        ...
        let first_add = harness
            .server
            .handle_request(tool_call(10, "agentpalace_kg_add", add_args.clone()))
            .await;
        let first_add_payload = decode_tool_payload(&first_add).unwrap();
        assert_eq!(first_add_payload["success"], true);
        let second_add =
            harness.server.handle_request(tool_call(11, "agentpalace_kg_add", add_args)).await;
        ...
        let first_invalidate = harness
            .server
            .handle_request(tool_call(12, "agentpalace_kg_invalidate", invalidate_args.clone()))
            .await;
        ...
        let second_invalidate = harness
            .server
            .handle_request(tool_call(13, "agentpalace_kg_invalidate", invalidate_args))
            .await;
    ...
    async fn staged_kg_reconciliation_determines_current_state_across_all_historical_facts() {
        ...
        let _f1_id = kg_runtime
            .add_fact(
                agentpalace_graph::AddFactRequest {
                    subject: "PersonA".into(),
                    subject_type: agentpalace_graph::EntityKind::Person,
                    predicate: "lives_in".into(),
                    object: "CityA".into(),
                    object_type: agentpalace_graph::EntityKind::Concept,
    ...
    /// mirroring the write:both `agentpalace_kg_add` flow's stage-before-commit ordering.
    ...
    fn make_base_config(palace_path: &std::path::Path, tempdir: &TempDir) -> AgentPalaceConfig {
        AgentPalaceConfig {
            schema_version: 1,
            collection_name: "agentpalace_drawers".to_owned(),

...
//! AgentPalace ships no pushed metrics sink (no Prometheus/OpenTelemetry exporter
...
//! `agentpalace_status` under `replication.metrics.phases`, while each sample is
//! also emitted as a structured `tracing` event (`target: "agentpalace_metrics"`)
...
impl PhaseMeter {
    ...
    /// `target = "agentpalace_metrics"`, fields `phase`, `elapsed_ms`, and `count`.

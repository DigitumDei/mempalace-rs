...
//!
//! Each test spawns a REAL federation HTTP server (agentpalace-server crate) in-process
...
//!
//! Spawning pattern mirrors `crates/agentpalace-remote/tests/remote_client_e2e.rs`.
//! MCP call pattern mirrors `crates/agentpalace-mcp/tests/stdio_harness.rs`.
...
use agentpalace_federation::{DrawerSearchRequest, KgQueryRequest};
use agentpalace_mcp::{DeterministicStubProvider, JsonRpcRequest, McpServer, decode_tool_payload};
...
/// Build a base `AgentPalaceConfig` rooted at `dir` with no federation (server side).
fn server_config(dir: &TempDir) -> AgentPalaceConfig {
    AgentPalaceConfig {
        schema_version: 1,
...
/// via the hub client); for terminal outcomes it is `agentpalace_status` →
...
/// Return the newest `agentpalace_status` payload.
async fn status_of(server: &McpServer<DeterministicStubProvider>) -> Value {
    call_tool(server, 900, "agentpalace_status", json!({})).await
}
...
/// Poll `agentpalace_status` until `predicate` holds; panics after `timeout`.
...
async fn federated_kg_reads_return_empty_for_unknown_entities() {
    ...
    let response =
        call_tool(&server, 1, "agentpalace_kg_query", json!({"entity": "unknown federation entity"}))
            .await;
...
async fn add_remote_search_combined_delete_remote_roundtrip() {
    ...
    let delete_resp =
        call_tool(&server, 5, "agentpalace_delete_drawer", json!({"drawer_id": remote_drawer_id}))
            .await;
...
async fn different_embedding_profiles_per_side() {
    ...
    let local_config = AgentPalaceConfig {
        schema_version: 1,
        collection_name: "agentpalace_drawers".to_owned(),
        palace_path: local_dir.path().join("palace"),
...
/// search still returns local results plus a "warnings" entry. `agentpalace_status`
/// reports the remote with "reachable": false. `agentpalace_kg_query` returns local
/// facts plus warnings.
#[tokio::test]
async fn remote_down_degrades_reads() {
    ...
    // agentpalace_status → federation.remotes[].reachable must be false.
    let status = call_tool(&server, 4, "agentpalace_status", json!({})).await;
    let remotes_info =
    ...
    // agentpalace_kg_query with dead remote → local facts + warnings.
...
async fn diary_room_never_routes_remote() {
    ...
    // `agentpalace_add_drawer` with room=diary uses ingest_mode="mcp_write", not
    // "diary", so `agentpalace_diary_read` won't find it. Instead we confirm the
    // draw landed in the LOCAL palace by querying `agentpalace_get_taxonomy`, which
    ...
    //
    // We use a simpler check: verify that agentpalace_list_wings includes
    // wing_diary_test in the merged result (hub contributes 0, local contributes 1).
    let list_wings = call_tool(&server, 2, "agentpalace_list_wings", json!({})).await;
...
/// With default_mode=Local and one wing ruled Remote at "hub", `agentpalace_status`
...
async fn wing_availability_reflects_rules() {
    ...
    // agentpalace_status must include wing_availability.
    let status = call_tool(&server, 2, "agentpalace_status", json!({})).await;
    ...
    // Also verify via agentpalace_list_wings (which also returns wing_availability).
    let list_wings = call_tool(&server, 3, "agentpalace_list_wings", json!({})).await;
...
/// Seed 2 drawers on the hub via routed adds, then call `agentpalace_wake_up`.
...
/// `agentpalace_wake_up` must still succeed (identity present) and
...
async fn wake_up_with_down_remote_marks_unreachable_and_succeeds() {
    ...
    let wake = call_tool(&server, 1, "agentpalace_wake_up", json!({"agent_name": "down-test"})).await;
...
/// Seed 3 drawers on the hub via routed adds. Call `agentpalace_get_changes_since`
...
/// Federated `agentpalace_get_changes_since` must show the drawer event with
...
/// `agentpalace_status`. Both the queued immediate response and the terminal outbox record are
/// asserted.
#[tokio::test]
...
/// failure** surfaced via `agentpalace_status` — not marked converged.
#[tokio::test]
...
async fn add_drawer_both_retry_reuses_local_drawer_and_replicates() {
    ...
    let config_a = AgentPalaceConfig {
        schema_version: 1,
        collection_name: "agentpalace_drawers".to_owned(),
        palace_path: local_dir.path().join("palace"),
        embedding_profile: EmbeddingProfile::Balanced,
    ...
    let config_b = AgentPalaceConfig {
        schema_version: 1,
        collection_name: "agentpalace_drawers".to_owned(),
        palace_path: local_dir.path().join("palace"),
        embedding_profile: EmbeddingProfile::Balanced,
    ...
    drop(server_b);
    let local_only_config = AgentPalaceConfig {
        schema_version: 1,
        collection_name: "agentpalace_drawers".to_owned(),
        palace_path: local_dir.path().join("palace"),
        embedding_profile: EmbeddingProfile::Balanced,
    ...
    .await
    .unwrap();
    let local_status = call_tool(&local_only_server, 1, "agentpalace_list_wings", json!({})).await;
...
async fn delete_both_retry_after_restart_replays_same_operation() {
    ...
    let make_config = || AgentPalaceConfig {
        schema_version: 1,
        collection_name: "agentpalace_drawers".to_owned(),
        palace_path: local_dir.path().join("palace"),
        embedding_profile: EmbeddingProfile::Balanced,
...
/// and `agentpalace_kg_query` with `as_of: "2026-07-01"` must NOT include the fact.
#[tokio::test]
...
/// rejection and records the operation as a terminal failure via `agentpalace_status`.
#[tokio::test]
...
/// `agentpalace_task_create` for a wing whose `federation.coordination` rule is `Remote` must
...
async fn coordination_task_create_routes_to_remote_when_wing_configured_remote() {
    ...
    let local_get =
        call_tool(&server, 2, "agentpalace_task_get", json!({"task_id": response["task_id"]})).await;
...
async fn coordination_task_create_omitted_wing_uses_configured_remote_default() {
    ...
    let observed = call_tool(&server, 2, "agentpalace_task_get", json!({"task_id": task_id})).await;
...
async fn coordination_task_create_omitted_wing_builtin_fallback_stays_local() {
    ...
    let local = call_tool(&server, 2, "agentpalace_task_get", json!({"task_id": task_id})).await;
...
async fn coordination_wings_discovery_matches_empty_configured_wing_placement() {
    ...
    let discovery = call_tool(&server, 1, "agentpalace_coordination_wings", json!({})).await;
...
/// server entirely) is invisible locally, but `agentpalace_task_get` still finds it by falling
...
async fn coordination_task_get_falls_back_to_remote_after_local_miss() {
    ...
    let hub_task = hub_client
        .coordination_task_create(agentpalace_federation::NewTaskRequest {
    ...
    // in `crates/agentpalace-mcp/src/federation.rs`.
    let server = mcp_server_with_hub_coordination(
        &local_dir,
        &hub_url,
        BTreeMap::new(),
    ...
    let response =
        call_tool(&server, 1, "agentpalace_task_get", json!({"task_id": hub_task.task_id})).await;
    ...
    let missing =
        call_tool(&server, 2, "agentpalace_task_get", json!({"task_id": "task_does_not_exist"}))
            .await;
...
/// `crates/agentpalace-mcp/src/lib.rs`, not this test's.
#[tokio::test]
async fn coordination_events_fanout_with_one_remote_down_still_returns_the_healthy_one() {
    ...
    let response =
        call_tool(&server, 1, "agentpalace_coordination_events", json!({"wing": "wing_fanout"}))
            .await;
...
/// Regression for Codex finding 3832912235: `agentpalace_coordination_events` reads a per-remote
...
async fn task_list_remote_discovery_continuation_and_claim() {
    ...
    .await;
    let page =
        call_tool(&server, 1, "agentpalace_task_list", json!({"wing":"task_pages","limit":2})).await;
    ...
    let next = call_tool(&server,2,"agentpalace_task_list",json!({"wing":"task_pages","limit":2,"include_local":false,"remotes":["hub"],"remote_cursors":{"hub":remote["next_cursor"]}})).await;
    ...
    let claimed = call_tool(&server,3,"agentpalace_task_claim",json!({"task_id":row["task_id"],"expected_revision":row["revision"],"worker":"worker","lease_seconds":60})).await;
...
async fn coordination_events_remote_cursors_round_trip_paginates_without_repeats() {
    ...
    for i in 0..2 {
        hub_client
            .coordination_message_send(agentpalace_federation::NewMessageRequest {
...
/// Companion to the events round trip above, for `agentpalace_inbox_read`'s equally undeclared
...
// e2e coverage above, but `agentpalace_message_send`, `agentpalace_message_acknowledge`,
// `agentpalace_artifact_put`, `agentpalace_artifact_get`, `agentpalace_result_put` and
// `agentpalace_result_get` do not — the unit tests in `federation.rs` exercise the *shared*
...
/// `agentpalace_message_send` falls back to the hub when the referenced task exists only there,
...
/// `agentpalace_message_acknowledge` falls back to the hub for a message that exists only there,
...
/// `agentpalace_artifact_put` falls back to the hub when the referenced task exists only there,
...
/// `agentpalace_artifact_get` falls back to the hub for an artifact that exists only there, and
...
async fn coordination_artifact_get_fallback_returns_correct_fields() {
    ...
    let hub_artifact = hub_client
        .coordination_artifact_put(agentpalace_federation::NewArtifactRequest {
            task_id: hub_task.task_id.clone(),
            role: "log".to_owned(),
...
/// `agentpalace_result_put` falls back to the hub when the referenced task exists only there, and
...
/// `agentpalace_result_get` falls back to the hub for a result that exists only there, and must
...
async fn coordination_result_get_fallback_returns_correct_payload() {
    ...
    let hub_result = hub_client
        .coordination_result_put(agentpalace_federation::NewTaskResultRequest {
            task_id: hub_task.task_id.clone(),
    ...
    let response =
        call_tool(&server, 1, "agentpalace_result_get", json!({"result_id": hub_result.result_id}))
            .await;

//! HTTP client for the AgentPalace federation REST API.
//!
//! This crate provides:
//!
...
//!
//! ```rust,no_run
//! use agentpalace_remote::{RemoteClient, RemoteEndpoint, RemoteApi, DEFAULT_TIMEOUT};
//!
//! # async fn example() -> agentpalace_remote::Result<()> {
...
/// and retry — mirroring `agentpalace_storage::RevisionedWrite<T>` (Phase 3 Stage 4) so a
/// revision conflict has one shape whether it came from local storage or a remote peer.
///
/// This crate deliberately does not depend on `agentpalace-storage` (it is meant to stay a
/// lightweight HTTP client), so this is its own type rather than a re-export; the routing layer
/// (`agentpalace-mcp::federation::FederationRouter`), which already depends on both crates, is
...
/// `FederationRouter`'s coordination fallback loops in `agentpalace-mcp::federation` route only
...
///
/// Plain values — the routing layer maps `agentpalace_config::ResolvedRemote`
...
pub trait RemoteApi: Send + Sync {
    ...
    /// Return whether this peer advertises receipt-backed idempotent mutations.
    ///
    /// `Some(false)` is authoritative and means operation-aware retries must not be sent. The
    /// default `None` keeps older in-process [`RemoteApi`] implementations source-compatible;
    /// concrete clients that perform the `/v1/info` handshake override it with the peer's
    /// capability bit.
    ...
    // `agentpalace-mcp::federation` and `agentpalace-mcp::lib`. Defaults also let a Stage-4-aware
    ...
    // fallback loops (`agentpalace-mcp::federation`) treat only a `404` and `CapabilityMissing`
    ...
    // `crates/agentpalace-mcp/src/federation.rs`.
    ...
    /// revision, not as an `Err` — matching how `agentpalace_storage::CoordinationStore` itself
    ...
    /// revision pair to report and stays an `Err`. AgentPalace never retries a conflicting write

...
/// Build a `AgentPalaceConfig` wired to `tempdir` with stub embeddings.
fn test_config(tempdir: &TempDir) -> AgentPalaceConfig {
    AgentPalaceConfig {
        schema_version: 1,
...
async fn spawn_server(config: AgentPalaceConfig, token_file: PathBuf) -> SocketAddr {
...
async fn round_trip_drawers_and_changes() {
    ...
    let search_resp = client
        .search_drawers(agentpalace_federation::DrawerSearchRequest {
    ...
    let req = IngestBatchRequest {
        ...
        files: vec![
            ...
            IngestFileDto {
                ...
                file_hash: Some(agentpalace_core::hash_bytes(beta_text.as_bytes())),
                chunks: vec![IngestChunkDto {
                    chunk_index: 0,
                    room: "code".to_owned(),
...
async fn coordination_round_trip() {
    ...
    let task = client
        ...
        .await
        .unwrap();
    assert_eq!(task.state, agentpalace_federation::CoordinationTaskState::Pending);
    ...
    let transitioned = match client
        .coordination_task_transition(
            &task.task_id,
            agentpalace_federation::TransitionTaskRequest {
    ...
    assert_eq!(transitioned.state, agentpalace_federation::CoordinationTaskState::Completed);

    let events = client
        .coordination_events(agentpalace_federation::CoordinationEventsQuery {
            cursor: None,
    ...
    let inbox = client
        .coordination_inbox(agentpalace_federation::InboxQuery {
            recipient: "manager".to_owned(),
            wing: None,
            cursor: None,
            limit: None,

//! Federation HTTP server for AgentPalace.
//!
//! Exposes a AgentPalace palace over a REST API with bearer-token authentication.
//! All wire types are defined in `agentpalace-federation`; this crate provides
...
//! let config: AgentPalaceConfig = todo!("load from disk");
...
use agentpalace_graph::{AddFactRequest, EntityKind, KnowledgeGraphRuntime, QueryDirection};
...
/// years. `agentpalace-storage`'s `claim_task`/`renew_lease` reject a TTL that would overflow
...
impl AuthIdentity {
    ...
    /// `resolve_kg_route` in `agentpalace-config`, which deliberately skips the
...
pub struct ServerState<P> {
    /// AgentPalace configuration.
    pub config: AgentPalaceConfig,
    ...
    /// in `agentpalace-mcp`. Coordination routes are server-only as of issue
...
/// use agentpalace_core::EmbeddingProfile;
///
/// let config: AgentPalaceConfig = todo!();
/// let tokens = TokenRegistry::load(std::path::PathBuf::from("server_tokens.json"))?;
/// let provider = DeterministicStubProvider::new(EmbeddingProfile::Balanced);
/// let (router, _state) = build_router(config, provider, tokens).await?;
...
fn summary_to_status(summary: &MaintenanceRunSummary) -> MaintenanceStatus {
    ...
    let fed_status = match summary.status {
        agentpalace_storage::MaintenanceRunStatus::Success => FedMaintenanceRunStatus::Success,
        agentpalace_storage::MaintenanceRunStatus::Partial => FedMaintenanceRunStatus::Partial,
        agentpalace_storage::MaintenanceRunStatus::Failure => FedMaintenanceRunStatus::Failure,
    };
    ...
    P: EmbeddingProvider + Send + Sync + 'static,
{
    ...
    let facts = match runtime.query_entity(&body.entity, as_of, direction) {
        Ok(facts) => facts,
        Err(agentpalace_graph::GraphError::UnknownEntity { .. }) => Vec::new(),
...
async fn validate_ingest_checkout(
    ...
    request: &agentpalace_federation::IngestPreflightRequest,
) -> Result<agentpalace_federation::IngestPreflightResponse, ServerError> {
    use agentpalace_federation::IngestPreflightResponse;
...
async fn route_ingest_preflight<P>(
    State(state): State<Arc<ServerState<P>>>,
    auth: axum::extract::Extension<AuthIdentity>,
    Json(mut body): Json<agentpalace_federation::IngestPreflightRequest>,
) -> Result<Json<agentpalace_federation::IngestPreflightResponse>, ServerError>
where
    P: EmbeddingProvider + Send + Sync + 'static,
{
    let wing = WingId::new(&body.wing)?;
    ...
    P: EmbeddingProvider + Send + Sync + 'static,
{
        ...
        match state.storage.receipt_store().begin_receipt(&NewReceipt {
            ...
            ReceiptOutcome::Replay(receipt) => {
                let response = serde_json::from_value(receipt.response.unwrap_or(Value::Null))
                    .map_err(agentpalace_storage::StorageError::from)?;
                return Ok(Json(response));
            }
            ReceiptOutcome::Conflict { .. } => {
    ...
    let preflight = agentpalace_federation::IngestPreflightRequest {
        ...
        files: body
            ...
            .filter_map(|file| {
                file.file_hash.as_ref().map(|file_hash| agentpalace_federation::IngestPreflightFile {
...
fn owning_task_wing(
    ...
    Ok(coordination
        .get_task(task_id)?
        .ok_or_else(|| {
            ServerError::Storage(agentpalace_storage::StorageError::Invariant(format!(
...
/// the request ever reaches storage. `agentpalace-storage`'s `claim_task` and
...
/// fragment matched below is re-exported from `agentpalace_storage::coordination`,
...
/// no clock at all (see the coordination-DTO docs in `agentpalace-federation`).
...
async fn route_coordination_tasks<P>(
    State(state): State<Arc<ServerState<P>>>,
    auth: axum::extract::Extension<AuthIdentity>,
    Query(params): Query<agentpalace_federation::CoordinationTasksQuery>,
) -> Result<impl IntoResponse, ServerError>
where
    P: EmbeddingProvider + Send + Sync + 'static,
{
    ...
    let filter = agentpalace_storage::TaskListFilter {
        ...
        state: params
            .state
            .as_deref()
            .map(agentpalace_storage::TaskState::parse)
            .transpose()
            .map_err(coordination_storage_error)?,
        owner: params.owner,
    ...
    let page = state
        ...
        .tasks(
            ...
            params.byte_budget.unwrap_or(agentpalace_storage::TASK_LIST_PAGE_BYTES),
            scope.visibility(),
        )
        .map_err(coordination_storage_error)?;
    Ok(Json(page))
...
/// wing (older events, and some remote-fallback paths in `agentpalace-mcp`,
...
/// (`crates/agentpalace-core/src/hash.rs`) embed `{wing}/{room}/...` directly.
    ...
    P: EmbeddingProvider + Send + Sync + 'static,
{
    ...
    let embedding = response.vectors().first().cloned().ok_or_else(|| {
        ServerError::Embeddings(agentpalace_embeddings::EmbeddingError::ProviderContract(
...
fn mutation_request_hash(fields: &[(&str, serde_json::Value)]) -> String {
    ...
    agentpalace_core::hash_text(&canonical)
}

/// Canonical KG label used as the stable target identity for a triple.
///
/// Mirrors `agentpalace-graph`'s private `canonicalize_label` so effect-inspection can compare
...
///
/// Mirrors the logic in `agentpalace-mcp`.
...
mod tests {
    ...
    async fn add_with_operation_id_replays_success_and_conflicts_on_different_request() {
        ...
        assert_eq!(receipt.status, agentpalace_storage::ReceiptState::Completed);
    ...
    async fn delete_fresh_keyed_absent_target_404s_and_retry_cannot_fabricate_success() {
        ...
        assert_eq!(receipt.status, agentpalace_storage::ReceiptState::Pending);
        assert!(
    ...
    async fn kg_add_recovers_pending_receipt_with_preapplied_effect() {
        ...
        assert_eq!(
            harness.state.storage.receipt_store().get_receipt(op).unwrap().unwrap().status,
            agentpalace_storage::ReceiptState::Pending
        );
    ...
    async fn kg_receipt_with_long_fields_succeeds_without_exceeding_target_length_limit() {
        ...
        assert_eq!(inv_receipt.status, agentpalace_storage::ReceiptState::Completed);
    ...
    async fn changes_hides_wingless_event_from_scoped_token_but_not_unrestricted() {
        ...
        // agentpalace-mcp remote-fallback delete path, could already contain.
        harness
            .state
            .storage
            .operational_store()
    ...
    async fn search_pagination_reaches_full_page_when_visible_results_rank_below_invisible() {
        ...
        // agentpalace-search), and "wing_0hidden" < "wing_alpha"
    ...
    async fn ingest_batch_checkout_mapped_resolves_non_stale() {
        use agentpalace_core::hash_bytes;
    ...
    async fn maintenance_runs_after_one_idle_period() {
        use agentpalace_storage::{MaintenanceOutcome, MaintenanceSkipReason};
    ...
    async fn scheduler_task_spawned_by_build_router_not_by_engine_open() {
        ...
        let settings = agentpalace_storage::MaintenanceSettings {
            enabled: true,
            idle_secs: 0,
            ..agentpalace_storage::MaintenanceSettings::default()
        };
    ...
    /// `agentpalace-storage`, which does the same thing for the storage-level tests).
    #[tokio::test]

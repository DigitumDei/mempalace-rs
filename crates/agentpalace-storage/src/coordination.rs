...
// `crates/agentpalace-server/src/lib.rs`) matches on these exact constants to decide whether a
...
/// signal `agentpalace-mcp`'s `is_local_record_missing` and `agentpalace-server`'s
...
pub enum CoordinationVisibility<'a> {
    ...
    /// ([`agentpalace_core::SHARED_AGENT_DIARY_WING`]).
    ///
...
"#)?;
    ...
    /// directly from `agentpalace_task_create`'s MCP arguments, and a new field there would
...
"#;
    ...
    /// built from the matching `pub const` — the same constants [`agentpalace-server`'s
    /// `coordination_storage_error`] matches on to classify HTTP status, and (for
    /// `NOT_FOUND_SUFFIX` specifically) [`agentpalace-mcp`'s `is_local_record_missing`] matches on
    ...
    fn conflict_error_messages_start_with_their_pinned_constants() {
        ...
        // `agentpalace-mcp`'s `is_local_record_missing` and `agentpalace-server`'s

...
use agentpalace_core::{DrawerId, DrawerRecord, EmbeddingProfile};
...
mod tests {
    ...
    use agentpalace_core::{DrawerId, DrawerRecord, EmbeddingProfile, RoomId, WingId};
    ...
    async fn reconcile_cleans_up_diary_summary_for_stale_diary_run() {
        let tempdir = tempdir().unwrap();
        let engine = StorageEngine::open(tempdir.path(), EmbeddingProfile::Balanced).await.unwrap();
        let drawer_id = agentpalace_core::DrawerId::new("diary_wing_test_0001").unwrap();
    ...
    async fn reconcile_preserves_summary_for_committed_drawer() {
        let tempdir = tempdir().unwrap();
        let engine = StorageEngine::open(tempdir.path(), EmbeddingProfile::Balanced).await.unwrap();
        let drawer_id = agentpalace_core::DrawerId::new("diary_wing_test_0010").unwrap();
        ...
        diary_record.wing = agentpalace_core::WingId::new("wing_agents").unwrap();
        diary_record.room = agentpalace_core::RoomId::new("diary").unwrap();
    ...
    async fn commit_diary_ingest_success() {
        let tempdir = tempdir().unwrap();
        let engine = StorageEngine::open(tempdir.path(), EmbeddingProfile::Balanced).await.unwrap();
        let drawer_id = agentpalace_core::DrawerId::new("diary_wing_test_0002").unwrap();
        let wing = agentpalace_core::WingId::new("wing_agents").unwrap();
        let room = agentpalace_core::RoomId::new("diary").unwrap();
    ...
    async fn commit_diary_ingest_cleans_up_on_failure() {
        let tempdir = tempdir().unwrap();
        let engine = StorageEngine::open(tempdir.path(), EmbeddingProfile::Balanced).await.unwrap();
        let drawer_id = agentpalace_core::DrawerId::new("diary_wing_test_0003").unwrap();
        ...
        existing.wing = agentpalace_core::WingId::new("wing_agents").unwrap();
        existing.room = agentpalace_core::RoomId::new("diary").unwrap();
    ...
    async fn reconcile_cleans_up_stale_failed_diary_run() {
        let tempdir = tempdir().unwrap();
        let engine = StorageEngine::open(tempdir.path(), EmbeddingProfile::Balanced).await.unwrap();
        let drawer_id = agentpalace_core::DrawerId::new("diary_wing_failed_0001").unwrap();
        ...
        staged.wing = agentpalace_core::WingId::new("wing_agents").unwrap();
        staged.room = agentpalace_core::RoomId::new("diary").unwrap();
        ...
        for batch in 0..64 {
            let drawers: Vec<_> = (0..4)
                .map(|i| {
                    ...
                    r.wing = agentpalace_core::WingId::new("wing").unwrap();
                    r.room = agentpalace_core::RoomId::new("room").unwrap();
                    r
                })
                .collect();

...
use agentpalace_core::{DrawerId, DrawerRecord, EmbeddingProfile, WingId};
    ...
    async fn count_by_wing_room(
        ...
        if exclude_diary {
            ...
            predicate.push_str(&format!(
                "wing != '{}' AND room != '{}'",
                escape_sql(agentpalace_core::SHARED_AGENT_DIARY_WING),
                escape_sql(agentpalace_core::DIARY_ROOM)
            ));
        }
        ...
        while let Some(batch) = stream.try_next().await? {
            for candidate in layer_metadata_from_batch(&batch)? {
                let position = top.partition_point(|existing| {
                    !agentpalace_core::compare_layer_drawers(existing, &candidate).is_gt()
                });
                if position < limit {
    ...
    (0..batch.num_rows())
        .map(|row| {
            Ok(DrawerRecord {
                ...
                room: agentpalace_core::RoomId::new(rooms.value(row))?,
    ...
    for batch in batches {
        ...
        for row in 0..batch.num_rows() {
            ...
            records.push(DrawerRecord {
                ...
                wing: agentpalace_core::WingId::new(wing.value(row))?,
                room: agentpalace_core::RoomId::new(room.value(row))?,
...
fn build_locator(
...
) -> Option<agentpalace_core::SourceLocator> {
    ...
    Some(agentpalace_core::SourceLocator {
        byte_start: bs,
        byte_end: be,
        line_start: ls,
        line_end: le,
...
fn build_view_metadata(
...
) -> Option<agentpalace_core::RepositoryViewMetadata> {
    ...
    Some(agentpalace_core::RepositoryViewMetadata {
        repo_id: rid,
...
mod tests {
    ...
    async fn list_drawers_returns_all_rows_beyond_ten_thousand() {
        ...
        expected.sort_by(agentpalace_core::compare_layer_drawers);
    ...
    async fn schema_migration_adds_locator_columns_to_old_table() {
        ...
        use lancedb::database::CreateTableMode;
        use agentpalace_core::SourceLocator;

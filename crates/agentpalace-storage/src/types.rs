...
pub struct DrawerFilter {
    ...
    /// reach them (see `route_drawers_list` in `agentpalace-server`). An empty
...
pub trait DrawerStore: Send + Sync {
    ...
    async fn count_by_wing_room(
        ...
        for drawer in self.list_drawers(&filter).await? {
            if exclude_diary
                && (drawer.wing.as_str() == agentpalace_core::SHARED_AGENT_DIARY_WING
                    || drawer.room.as_str() == agentpalace_core::DIARY_ROOM)
            {
                continue;
            }
            *counts

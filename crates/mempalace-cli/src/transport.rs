...
use mempalace_embeddings::{EmbeddingProvider, env_flag};
use mempalace_mcp::{
    DeterministicStubProvider, McpServer, configured_lineage_id_from_env, default_provider,
    serve_transport,
};
use mempalace_server::TokenRegistry;
...
pub struct SharedProvider<P> {
    inner: Arc<Mutex<P>>,
    profile: &'static mempalace_core::EmbeddingProfileMetadata,
}
...
impl<P: EmbeddingProvider> EmbeddingProvider for SharedProvider<P> {
    ...
    fn startup_validation(
        &self,
    ) -> mempalace_embeddings::Result<mempalace_embeddings::StartupValidation> {
        self.inner
            .lock()
            .map_err(|_| {
                mempalace_embeddings::EmbeddingError::ProviderContract(
    ...
    fn embed(
        &mut self,
        request: &mempalace_embeddings::EmbeddingRequest,
...
pub async fn stdio(config: MempalaceConfig) -> Result<(), Error> {

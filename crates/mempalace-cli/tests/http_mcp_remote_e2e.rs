...
//! Set MEMPALACE_TEST_BINARY and MEMPALACE_TEST_REAL_EMBEDDINGS=1 to smoke-test a deployment.
...
impl Server {
    fn start(config: Value) -> Self {
        ...
        let binary = std::env::var_os("MEMPALACE_TEST_BINARY")
            .unwrap_or_else(|| env!("CARGO_BIN_EXE_mempalace").into());
        let real = std::env::var("MEMPALACE_TEST_REAL_EMBEDDINGS").as_deref() == Ok("1");
        let child = Command::new(binary)
            .args(["serve", "--bind", "127.0.0.1:0", "--token-file"])
            .arg(tokens)
            .env("MEMPALACE_CONFIG_DIR", dir.path())
            .env("MEMPALACE_PALACE_PATH", dir.path().join("palace"))
            .env("MEMPALACE_STUB_EMBEDDINGS", if real { "0" } else { "1" })
            .env("MEMPALACE_EMBED_ALLOW_DOWNLOADS", "0")
    ...
    async fn tool(&self, client: &reqwest::Client, name: &str, arguments: Value) -> Value {
        ...
        mempalace_mcp::decode_tool_payload(&response).unwrap_or_else(|| panic!("{response}"))
    }
...
async fn http_mcp_routes_to_remote_and_survives_remote_shutdown() {
    ...
    let search = gateway
        .tool(&client, "mempalace_search", json!({"query":content,"wing":"wing_remote","limit":5}))
        .await;
    ...
    let local = gateway.tool(&client, "mempalace_add_drawer", json!({
    ...
    let deleted = gateway.tool(&client, "mempalace_delete_drawer", json!({"drawer_id":id})).await;
    ...
    remote.stop();
    let wake = gateway
        .tool(&client, "mempalace_wake_up", json!({"agent_name":"remote-e2e","latest_limit":5}))
        .await;

...
`false` disable it. That is what makes the sandbox fully offline, but don't carry the
variable into an environment where you want real embeddings — the stub's vectors are not
comparable with model output.

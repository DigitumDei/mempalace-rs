...
|---|---|---|
| `MEMPALACE_CONFIG_DIR` | `mempalace-cli`, `mempalace-mcp` | Overrides the `~/.mempalace` base directory used to resolve `config.json`, `projects.json`, `people_map.json`, and (unless `server.token_file` is set) `server_tokens.json`. Unset or blank preserves today's `~/.mempalace` default. A directory that does not exist is not an error: it resolves the same way a fresh install would, with every file inside it treated as absent until something creates it there. It is independent of `MEMPALACE_PALACE_PATH`/`MEMPAL_PALACE_PATH` below: that variable always wins for the palace path specifically, even when `MEMPALACE_CONFIG_DIR` is also set — but if neither the env var nor `config.json`'s `palace_path` sets the palace explicitly, the default palace path is derived as `<MEMPALACE_CONFIG_DIR>/palace`, so setting only `MEMPALACE_CONFIG_DIR` relocates the palace too. See [Federation.md Part 6](Federation.md#part-6--dev-testing-locally) for the two-palace workflow this unblocks. |

...
All envelopes are UTF-8 JSON objects. Version `agentpalace.coordination/v1alpha1` is experimental and additive-only: consumers must ignore unknown fields but reject unknown `kind` or `api_version` values.

## Common fields

| Field | Requirement |
|---|---|
| `api_version` | Exactly `agentpalace.coordination/v1alpha1` |

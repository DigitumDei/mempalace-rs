...
5. Manager polls `agentpalace_get_changes_since` with every saved remote-origin cursor and uses the bounded overlapping local procedure in `SKILL.md`, validates candidate envelopes, de-duplicates by `idempotency_key`, and merges referenced results. Without get-by-ID, retrieval is an experimental search fallback and must be reported as unverified.
